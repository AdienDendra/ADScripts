...
class FindRenameDialog(QtWidgets.QDialog):
    ...
    def excuted_renaming_name(self):
        ...
        nondelete_node = self.nondelete_node()
        # if self.line_search_name_text() in items:
        #     pm.displayError("Object 'Find' is non-delete item. The process cannot be continued")
        # else:
        #     pass

        if get_search_object:
            if not get_search_object in nondelete_node:
