...
def show_menu():
    ...
    renamer_menu = pm.menuItem(label='Renamer', parent=custom_tools_menu,
                               c='import find_rename as sr \nsr.FindRenameDialog.show_ui()', tearOff=True)
    fixer_menu = pm.menuItem(label='Geo Test Scene Fixer', subMenu=True, parent=custom_tools_menu, tearOff=True)
    fix_present = pm.menuItem(label='Fix Present', parent=fixer_menu,
                              c='import birdy_test as bt \nreload(bt) \nbt.fix_present()', tearOff=True)
    look_at_gift = pm.menuItem(label='Look at Gift', parent=fixer_menu,
                               c='import birdy_test as bt \nreload(bt) \nbt.look_at_gift()', tearOff=True)
    put_on_party_hat = pm.menuItem(label='Put on Party Hat', parent=fixer_menu,
