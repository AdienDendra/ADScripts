...
def createFollicle(objSel, objMesh, conectFoll=['']):
    ...
    #rename follicle
    rename = mc.rename(follicleTransform, '%s_%s' % (ut.ad_lib_main_name(objSel), 'fol'))
...
def locator(obj):
    mc.select(cl=1)
    loc = mc.spaceLocator()
    rnm = mc.rename(loc, '%s_%s' % (ut.ad_lib_main_name(obj), 'loc'))
    cls = mc.cluster(obj)
...
def joint(obj):
    mc.select(cl=1)
    jnt = mc.joint()
    rnm = mc.rename(jnt, '%s_%s' % (ut.ad_lib_main_name(obj), 'jnt'))
    cls = mc.cluster(obj)
    mc.parentConstraint(cls, rnm, mo=0)
    mc.delete(cls)
    return obj
...
def groupFK(obj):
    for i in obj:
        splitName = i.split('|')[0]
        grpParent = ut.group_parent(['ParentPos', 'PosSDK'], ut.ad_lib_main_name(i), '')
