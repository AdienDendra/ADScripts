...
from rigging.library.utils import joint as rlu_joint, controller as rlu_controller, \
    rotationController as rlu_rotationController, \
    core as rlu_core
...
def loop(curve='', world_up_loc='', controller=False):
    ...
    create_ik = rlu_joint.joint_on_curve(curve=curve, world_up_loc=world_up_loc, delete_group=False,
                                         ctrl=controller)

    ctrl = rlu_controller.Control(match_obj_first_position=create_ik['joints'][0], prefix=au.prefix_name(curve),
    ...
    rlu_rotationController.change_position(ctrl.control, 'xy')
