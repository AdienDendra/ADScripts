...
class RibbonSpine:
    ...
    ### CREATING FOLLICLE AND PART OF IT
        ...
        # Match position to the spine
        mc.delete(mc.parentConstraint(listSpineJnt[0], listSpineJnt[5], self.grpGeoPlane, mo=0))
        mc.delete(mc.parentConstraint(listSpineJnt[0], listSpineJnt[5], self.grpGeoPlaneCls, mo=0))

    ### CREATING CLUSTER
        # Create group for replacing cluster position
        chestGrpCls  = au.group_follow_object(['Zro', 'Driver'], [listSpineJnt[5]], 'Cls')
        spine2GrpCls = au.group_follow_object(['Zro', 'Driver'], [listSpineJnt[3]], 'Cls')
        spine1GrpCls = au.group_follow_object(['Zro', 'Driver'], [listSpineJnt[2]], 'Cls')
        hipGrpCls    = au.group_follow_object(['Zro', 'Driver'], [listSpineJnt[1]], 'Cls')
        pelvisGrpCls = au.group_follow_object(['Zro', 'Driver'], [listSpineJnt[0]], 'Cls')

        # Grouping cluster postion grp to group misc
        mc.parent(chestGrpCls[0], spine2GrpCls[0], spine1GrpCls[0], hipGrpCls[0], pelvisGrpCls[0], self.grpCluster)

        # Freezing the rotate and translate of cluster group
        mc.makeIdentity(chestGrpCls[0], spine2GrpCls[0], spine1GrpCls[0], hipGrpCls[0], pelvisGrpCls[0], a=1, pn=1, r=1, t=1)


        # # Match position each cluster
        # mc.delete(mc.parentConstraint(listSpineJnt[5], chestRibbonClstr, mo=0))
        # mc.delete(mc.parentConstraint(listSpineJnt[3], spine2RibbonClstr, mo=0))
        ...
        chestRibbonCvs      = '%s.cv[0:3][5]' % geoPlaneCls[0]
        spine3RibbonCvs     = '%s.cv[0:3][4]' % geoPlaneCls[0]
        spine2RibbonCvs     = '%s.cv[0:3][3]' % geoPlaneCls[0]
        spine1RibbonCvs     = '%s.cv[0:3][2]' % geoPlaneCls[0]
        hipRibbonCvs        = '%s.cv[0:3][1]' % geoPlaneCls[0]
        pelvisRibbonCvs     = '%s.cv[0:3][0]' % geoPlaneCls[0]

        # # Create cluster
        # chestRibbonClstr  = mc.cluster(chestRibbonCvs,  wn=[chestGrpCls[1], chestGrpCls[1]],
        #                                bs=True, n=prefix + 'Chest_cls')
        # spine3RibbonClstr = mc.cluster(spine3RibbonCvs, wn=[chestGrpCls[1], chestGrpCls[1]],
        #                                bs=True, n=prefix + 'Spine3_cls')
        # spine2RibbonClstr = mc.cluster(spine2RibbonCvs, wn=[spine2GrpCls[1], spine2GrpCls[1]],
        #                                bs=True, n=prefix + 'Spine2_cls')
        # spine1RibbonClstr = mc.cluster(spine1RibbonCvs, wn=[spine1GrpCls[1], spine1GrpCls[1]],
        #                                bs=True, n=prefix + 'Spine1_cls')
        # hipRibbonClstr    = mc.cluster(hipRibbonCvs, wn=[hipGrpCls[1], hipGrpCls[1]],
        #                                bs=True, n=prefix + 'hip_cls')
        # pelvisRibbonClstr = mc.cluster(pelvisRibbonCvs, wn=[pelvisGrpCls[1], pelvisGrpCls[1]],
        #                                bs=True, n=prefix + 'Pelvis_cls')

        chestRibbonClstr  = mc.cluster(chestRibbonCvs,
                                       bs=True, n=prefix + 'Chest_cls')
        spine3RibbonClstr = mc.cluster(spine3RibbonCvs,
                                       bs=True, n=prefix + 'Spine3_cls')
        spine2RibbonClstr = mc.cluster(spine2RibbonCvs,
                                       bs=True, n=prefix + 'Spine2_cls')
        spine1RibbonClstr = mc.cluster(spine1RibbonCvs,
                                       bs=True, n=prefix + 'Spine1_cls')
        hipRibbonClstr    = mc.cluster(hipRibbonCvs,
                                       bs=True, n=prefix + 'hip_cls')
        pelvisRibbonClstr = mc.cluster(pelvisRibbonCvs,
                                       bs=True, n=prefix + 'Pelvis_cls')
        ...
        mc.sets(chestRibbonClstr, wn=[chestGrpCls[1], chestGrpCls[1]])
        mc.sets(spine3RibbonClstr, wn=[chestGrpCls[1], chestGrpCls[1]])
        mc.sets(spine2RibbonClstr, wn=[spine2GrpCls[1], spine2GrpCls[1]])
        mc.sets(spine1RibbonClstr, wn=[spine1GrpCls[1], spine1GrpCls[1]])
        mc.sets(hipRibbonClstr, wn=[hipGrpCls[1], hipGrpCls[1]])
        mc.sets(pelvisRibbonClstr, wn=[pelvisGrpCls[1], pelvisGrpCls[1]])
        ...
        # chestGrpClsOff  = mc.group(chestRibbonClstr, spine3RibbonClstr, n='%s%s_%s'% (au.prefixName(chestGrpCls[0])[:-3],'Offset','grp'))
        # spine2GrpClsOff = mc.group(spine2RibbonClstr, n='%s%s_%s'% (au.prefixName(spine2GrpCls[0])[:-3],'Offset','grp'))
        # spine1GrpClsOff = mc.group(spine1RibbonClstr, n='%s%s_%s'% (au.prefixName(spine1GrpCls[0])[:-3],'Offset','grp'))
        # hipGrpClsOff    = mc.group(hipRibbonClstr, n='%s%s_%s'% (au.prefixName(hipGrpCls[0])[:-3],'Offset','grp'))
        # pelvisGrpClsOff = mc.group(pelvisRibbonClstr, n='%s%s_%s'% (au.prefixName(pelvisGrpCls[0])[:-3],'Offset','grp'))

        # mc.parent(chestGrpClsOff, chestGrpCls[-1])
        # mc.parent(spine2GrpClsOff, spine2GrpCls[-1])
        # mc.parent(spine1GrpClsOff, spine1GrpCls[-1])
        # mc.parent(hipGrpClsOff, hipGrpCls[-1])
    ...
    ### SCALE CONSTRAINT FROM FOLLICLE PARENT CLS TO TO JOINT SPINE
        ...
        mc.scaleConstraint(follicleSetGrp[1], listSpineJnt[2])
        mc.scaleConstraint(follicleSetGrp[2], listSpineJnt[3])
    ...
    ### SCALE DECOMPOSE MATRIX FROM ANIM CONTROLLER TO FOLLICLE VOLUME
        ...
        mc.hide(self.grpCluster[0], self.grpGeoPlaneCls[0],self.grpDeform[0], self.grpDeformers[0],
                 self.grpFollVolume[0], self.grpParentFol[0])
