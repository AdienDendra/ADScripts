...
class Build:
    def __init__(self,
        ...
        # create parts directories
        # self.modulePart = md.Part(prefix=prefix, baseObj= md.Base())

    ### CREATE CONTROL PELVIS AND ROOT
        self.controllerRoot = ct.Control(match_obj_first_position=rootJnt, prefix='root', shape=ct.SQUAREPLUS,
    ...
    ### POINT CONSTRAINT ROOT AND SPINE IK UP TO SPINE IK LOW
        pointConsIKLow = mc.pointConstraint(self.controllerRoot.control_gimbal, self.controllerSpineIKUp.control_gimbal,
                                            self.controllerSpineIKLow.parent_control[0], mo=1)
        #mc.setAttr('%s.%s%s'% (pointConsIKLow[0], self.controllerSpineIKUp.controlGimbal,'W1'), 0.4)

        ### CREATE CONTROL FK/IK SETUP
        self.controllerFKIKSpineSetup = ct.Control(match_obj_first_position=rootJnt, prefix='spineSetup', shape=ct.STICKCIRCLE,
                                                   groups_ctrl=['Zro'], ctrl_size=scale,
                                                   ctrl_color='navy', lock_channels=['v', 't', 'r', 's'])

    ### CREATE CONTROL FLEX SPINE SETUP
        self.controllerFlexSpineSetup = ct.Control(match_obj_first_position=spineJnt[1], prefix='flexSetup', shape=ct.SQUAREPLUS,
                                                   groups_ctrl=['Zro'], ctrl_size=scale * 1.2,
                                                   ctrl_color='blue', lock_channels=['v', 's'])

        # parent flex spine to root gimbal ctrl
        mc.parent(self.controllerFlexSpineSetup.parent_control[0], self.controllerRoot.control_gimbal)
    ...
    ### ADD ATTRIBUTE FOR FK/IK SETUP CONTROLLER
        ...
        # add attribute flex spine setup
        au.add_attr_transform(self.controllerFKIKSpineSetup.control, 'flexSpineSetup', 'long', channel_box=True, min=0, max=1, dv=0)
        ...
        # set on/off attribute flex spine setup
        au.connect_part_object(obj_base_connection='flexSpineSetup', target_connection='visibility', obj_name=self.controllerFKIKSpineSetup.control,
                               target_name= [self.controllerFlexSpineSetup.parent_control[0]],
                               select_obj= False)
    ...
    ### ADD ATTRIBUTES ON CTRL FLEX SPINE AND FOR CONNECTING TO DEFORMER
        ...
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['squash'], nice_name=[' '], at="enum",
                         en='Squash IK', channel_box=True)

        au.add_attr_transform(self.controllerFlexSpineSetup.control, 'autoSquash', 'long', keyable=True, min=0, max=1, dv=0)

        # Add attributes: Volume attributes
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['volumeSep'], nice_name=[' '],
                         at="enum", en='Volume', channel_box=True)
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['volume'],
                         at="float", min=-1, max=1, keyable=True)
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['volumeMultiplier'],
                         at="float", min=1, dv=3, keyable=True)
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['startDropoff'],
                         at="float", min=0, max=1, dv=1, keyable=True)
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['endDropoff'],
                         at="float", min=0, max=1, dv=1, keyable=True)
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['volumeScale'],
                         at="float", min= -3* 0.5, max= 3 * 2, keyable=True)
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['volumePosition'],
                         min=-1.5, max=1.5, at="float", keyable=True)

        # Add attributes: Sine attributes
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['sineSep'], nice_name=[' '],
                         attributeType='enum', en="Sine:", channel_box=True)

        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['amplitude'],
                         attributeType="float", keyable=True)
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['offset'],
                         attributeType="float", keyable=True)
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['twist'],
                         attributeType="float", keyable=True)
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['sineLength'], min=0.1, dv=2,
                         attributeType="float", keyable=True)

        # Add attributes: Extra attributes
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['extraSep'], nice_name=[' '],
                         at="enum", en='Extra', channel_box=True)
        au.add_attribute(objects=[self.controllerFlexSpineSetup.control], long_name=['showRibbonCtrl'],
                         at="long", min=0, max=1, dv=1, channel_box=True)
