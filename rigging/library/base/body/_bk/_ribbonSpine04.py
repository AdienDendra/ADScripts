...
class RibbonSpine:
    ...
    ### GROUPS AND SURFACES
        ...
        self.grpCtrl         = mc.duplicate(self.grpAllRibbon, name=(prefix + 'RbnCtrl_grp'))
    ...
    ### CREATING JOINT TEMPORARY FOR SPLITTING JNT FOLLICLE
        ...
        # self.grpGeoPlane    = au.groupObject(['Zro'], geoPlane[0], geoPlane[0])
        ...
        follicleS =self.itemFollicle([listSpineJnt[1],listSpineJnt[2],listSpineJnt[3]], geoPlaneSkin, 'fol')
        ...
        for folS, folV, spineJnt in zip(follicleS['follicle'], follicleV['follicle'], listSpineJnt[1:4]):
            ...
            # allGrpSet = au.groupObject(['Scalar'], follicleGrpOffset, follicleGrpOffset)
            # print allGrpSet
            ...
            # follicleGrpOffsets = mc.parent(allGrpSet, folS)
            #
            ...
            # follicleCtrlGrp.append(follicleCtrl.parentControl[0])
            ...
            # Parent all set group to grp ctrl
            mc.parent(follicleGrpOffset, self.grpCtrl[0])

            ## Make it follicle connected joint for sine deformer purpose
            zroTransPma = mc.createNode('plusMinusAverage', n='%s%s'%(au.prefix_name(folS), 'ZroTrans_pma'))
            transPma    = mc.createNode('plusMinusAverage', n='%s%s'%(au.prefix_name(folS), 'Trans_pma'))
            rotPma      = mc.createNode('plusMinusAverage', n='%s%s'%(au.prefix_name(folS), 'Rot_pma'))

            # Connect from follicle to zroTransPma
            getAttrFol  = mc.getAttr(folS+'.translate')[0]
            mc.connectAttr(folS+'.translate', zroTransPma+'.input3D[0]', f=True)
            mc.setAttr(zroTransPma+'.input3D[1].input3Dx', getAttrFol[0]*-1)
            mc.setAttr(zroTransPma+'.input3D[1].input3Dy', getAttrFol[1]*-1)
            mc.setAttr(zroTransPma+'.input3D[1].input3Dz', getAttrFol[2]*-1)

            # Connect output zroTransPma to set group
            mc.connectAttr(zroTransPma +'.output3D', follicleCtrl.parent_control[0] + '.translate', f=True)

            # Connect follicle ctrl and group to trans pma and deliver to joint translate
            mc.connectAttr(follicleCtrl.parent_control[0] + '.translate', transPma + '.input3D[0]', f=True)
            mc.connectAttr(follicleCtrl.control+'.translate', transPma+'.input3D[1]', f=True)
            mc.connectAttr(transPma+'.output3D', spineJnt+'.translate', f=True)

            # Connect follicle ctrl and group to rotate pma and deliver to joint translate
            mc.connectAttr(follicleCtrl.parent_control[0] + '.rotate', rotPma + '.input3D[0]', f=True)
            mc.connectAttr(follicleCtrl.control+'.rotate', rotPma+'.input3D[1]', f=True)
            mc.connectAttr(rotPma+'.output3D', spineJnt+'.rotate', f=True)

            ### CONNECT ATTRIBUTE  SCALE FROM CONTROLLER FOLLICLE TO JOINTS
            mc.scaleConstraint(follicleCtrl.control, spineJnt)
    ...
    ### CONNECTING BETWEEN CONTROLLER TO SPINE RIBBON AND JOINTS DRIVER
     ## Create grp null for spine follicle 03
        ...
        mc.parent(ctrlFK3Null, ctrlIK3Null, self.grpExtraFol[0])
    ...
    ### SCALE DECOMPOSE MATRIX FROM ANIM CONTROLLER TO FOLLICLE AND SKIN JOINT
        ...
        for i, j in zip (follicleSetGrp, jntSkinList):
        ...
        mc.parent(self.grpCtrl[0], self.grpDeform[0], self.grpSurface[0], self.grpDeformers[0], self.grpFollMain[0],
