...
class CreateDetail:
    def __init__(self,
        ...
        self.jointUpParent   = tr.create_parent_transform(parent_list= ['Zro'], object=self.jointUp[0],
                                                          match_position=self.jointUp[0], prefix=prefix + 'Up',
                                                          suffix='_jnt', side=side)

        self.jointMidParent  = tr.create_parent_transform(parent_list=['Zro'], object=self.jointMid[0],
                                                          match_position=self.jointMid[0], prefix=prefix + 'Mid',
                                                          suffix='_jnt', side=side)

        self.jointDownParent = tr.create_parent_transform(parent_list=['Zro'], object=self.jointDown[0],
                                                          match_position=self.jointDown[0], prefix=prefix + 'Down',
                                                          suffix='_jnt', side=side)

        # Create the controllers
        self.ctrlUp   = ct.Control(match_obj_first_position=self.jointUpParent[0], prefix=prefix + 'Up', side=side,
                                   groups_ctrl=['Zro', 'Offset', 'Twist', 'Rotation'],
                                   ctrl_size=scale * 0.7, ctrl_color=ctrlColor, shape=ctrlTip)
        self.ctrlMid  = ct.Control(match_obj_first_position=self.jointMidParent[0], prefix=prefix + 'Mid', side=side,
                                   groups_ctrl=['Zro', 'Aim'],
                                   ctrl_color='blue', ctrl_size=scale * 0.7, shape=ctrlMid)
        self.ctrlDown = ct.Control(match_obj_first_position=self.jointDownParent[0], prefix=prefix + 'Down', side=side,
        ...
        follicleCtrlGrp        = []
        self.follicleGrpSetGrp = []
        self.follicleGrpTwist  = []
        self.follicleGrpOffset = []
        self.follicleCtrl      = []

        for i, folS in enumerate(follicleS['follicle']):
            ...
            follicleGrpSetGrp = mc.group(empty=True, n='%s%02d%s_%s' % (prefix,(i+1), side,'setGrp'))
            mc.delete(mc.parentConstraint(folS, follicleGrpSetGrp))

            # append the set grp
            self.follicleGrpSetGrp.append(follicleGrpSetGrp)

            # Create offset group follicle
            follicleGrpSetGrp = mc.parent(follicleGrpSetGrp, folS)
            ...
            follicleCtrl  = ct.Control(prefix=prefix + ('%02d' % (i+1)), groups_ctrl=['Zro', 'Twist', 'Offset']
                                       , side=side, ctrl_color=ctrlColor, ctrl_size=scale * 0.65, shape=ctrlDetails)
            ...
            self.follicleGrpOffset.append(follicleCtrl.parent_control[2])
            self.follicleCtrl.append(follicleCtrl.control)
            ...
            mc.delete(mc.pointConstraint(follicleGrpSetGrp, follicleCtrl.parent_control[0]))

            # Parent controller to group offset follicle
            mc.parent(follicleCtrl.parent_control[0], follicleGrpSetGrp)
    ...
    # ==================================================================================================================
        ...
        self.addAttribute(objects=[self.ctrlDown.control, self.ctrlUp.control], longName=['twistOffset'], at="float",
                          k=True)
        self.addAttribute(objects=[self.ctrlDown.control, self.ctrlUp.control], longName=['affectToMid'], at="float",
                          min=0,
                          max=10, dv=10, k=True)

        # Add attributes: Twist/Roll attributes parent
        self.addAttribute(objects=[self.ctrlDown.parent_control[2], self.ctrlUp.parent_control[2]], longName=['twist'],
                          at="float", k=True)
        self.addAttribute(objects=[self.ctrlDown.parent_control[2], self.ctrlUp.parent_control[2]],
                          longName=['twistOffset'], at="float", k=True)
        self.addAttribute(objects=[self.ctrlDown.parent_control[2], self.ctrlUp.parent_control[2]],
                          longName=['affectToMid'], at="float", k=True)
        ...
        mc.blendShape(dtlBlendshp, e=True, w=[(1, 1.0)])
    ...
    # ==================================================================================================================
        ...
        #     mc.blendShape(dtlBlendshp, e=1, t=(geoPlane[0], 1, geoPlaneTwist[0], 1.0))
            ...
            mc.blendShape(dtlBlendshp, e=True, w=[(2, 1.0)])
            ...
            for folV, folGrpOffst in zip(follicleV['follicle'], self.follicleGrpSetGrp):
                # combine multiplier
                combineMultMpd = mc.shadingNode('multDoubleLinear', asUtility=1,
                                                name=au.prefix_name(folV)[:-3] + 'CombineMultiplier' + side + '_mdl')
            ...
            # Cleanup: Hierarchy
            mc.parent(self.sineDef[1], self.squashDef[1], self.grpDeformers)
            mc.parent(geoPlaneSine[0], geoPlaneVolume[0], self.grpSurfaces)
            mc.parent(follicleV['follicle'], self.grpFollVolume)
            mc.parent(self.grpFollVolume[0], self.grpNoTransform)

            # hide grp
            mc.hide(self.grpNoTransformZro)
    ...
    # ==================================================================================================================
        ...
        mc.parent(geoPlaneOrient[0], geoPlaneTwist[0], self.grpSurfaces)
        mc.parent(self.twistDef[1], self.grpDeformers)
        ...
        mc.parent(self.grpJntCluster, self.grpSurfaces[0], self.grpDeformers[0], self.grpMisc[0],  self.grpNoTransform)
        ...
        mc.delete(mc.parentConstraint(self.ctrlUp.parent_control[0], self.ctrlDown.parent_control[0],
                                      self.ctrlMid.parent_control[0]))

        # aim contraint for ctrl
        mc.aimConstraint(self.ctrlUp.control, self.downCtrlAimGrp, mo=1, w=1, aim=(0,-1, 0), u= (0,0,-1),
                         wut='objectrotation', wu= (0,0,-1), wuo=self.downCtrlWorldOffGrp)

        mc.aimConstraint(self.ctrlDown.control, self.upCtrlAimGrp, mo=1, w=1, aim=(0, 1, 0), u= (0,0,-1),
                         wut='objectrotation', wu= (0,0,-1), wuo=self.upCtrlWorldOffGrp)
        ...
        mc.parentConstraint(self.ctrlUp.control, self.ctrlDown.control, self.midCtrlWorldOffGrp, mo=1)
        ...
        # hide the visibility
        mc.hide(self.grpNoTransformZro, self.grpNoTransform, self.grpDeformers, self.grpJntCluster, geoPlaneWire,
                self.grpSurfaces, self.grpSurface, self.grpMisc)
