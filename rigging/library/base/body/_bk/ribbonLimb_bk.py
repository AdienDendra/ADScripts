...
class CreateRibbon:
    def __init__(self,
        ...
        :param tipPos       : str, position point (joint) of the tip regarding on the module joint by seeing the axis whether on + or - axis
        :param aimAxis      : str, aim axis is that tip pivot looking the module joint. Can fill with +x +y +z or -x -y -z
        ...
        self.jointMid = mc.duplicate(jointCreate, n=prefix + 'Mid_'+side +'jnt')
        ...
        self.jointOrientUp = mc.duplicate(jointCreate, n=prefix + 'OrientUp'+side +'_jnt')
        self.jointOrientMid = mc.duplicate(jointCreate, n=prefix + 'OrientMid'+side +'_jnt')
        self.jointOrientDown = mc.duplicate(jointCreate, n=prefix + 'OrientDown'+side +'_jnt')
        ...
        self.jointUpOrientParent = tr.create_parent_transform(['Zro'], self.jointOrientUp[0],
                                                              self.jointOrientUp[0])
        self.jointMidOrientParent = tr.create_parent_transform(['Zro'], self.jointOrientMid[0],
                                                               self.jointOrientMid[0])
        self.jointDownOrientParent = tr.create_parent_transform(['Zro'], self.jointOrientDown[0],
                                                                self.jointOrientDown[0])

        # Create the controllers
        self.ctrlUp = ct.Control(match_obj_first_position=self.jointUpParent[0], prefix=prefix + 'Up' + side, groups_ctrl=['Zro', 'Offset', 'Combine'],
                                 ctrl_size=scale * 0.7, shape=ctrlTip)
        ...
        self.ctrlDown = ct.Control(match_obj_first_position=self.jointDownParent[0], prefix=prefix + 'Down' + side, groups_ctrl=['Zro', 'Offset', 'Combine'],
                                   ctrl_size=scale * 0.7, shape=ctrlBase)

        # create grp for make aim constraint
        # control up
        ...
        # match orient joint
        mc.delete(mc.orientConstraint(base, self.jointUpOrientParent[0]))
        mc.delete(mc.orientConstraint(base, tip, self.jointMidOrientParent[0]))
        mc.delete(mc.orientConstraint(tip, self.jointDownOrientParent[0]))
        ...
        sumDoubleCtrlUp = mc.shadingNode('plusMinusAverage', asUtility=1, name=(prefix + 'AverageOrientUp'+side +'_pma'))
        mc.connectAttr((self.ctrlUp.control + '.rotate'), (sumDoubleCtrlUp + '.input3D[0]'))
        ...
        mc.connectAttr(self.ctrlMid.control + '.rotate', self.jointOrientMid[0] + '.rotate')

        # Connect controller tip and down joint orient
        sumDoubleCtrlDown = mc.shadingNode('plusMinusAverage', asUtility=1, name=(prefix + 'AverageOrientDwn'+side +'_pma'))
        mc.connectAttr((self.ctrlDown.control + '.rotate'), (sumDoubleCtrlDown + '.input3D[0]'))
        ...
        rbnBlendshp = mc.blendShape(geoPlaneWire[0],  geoPlane[0], name=(prefix + side+ '_bsn'), weight=[(0, 1)])
        ...
        decomposeMtxNode = mc.createNode('decomposeMatrix', n=prefix + 'RbnScaleFol'+side +'_dmtx')
        ...
        for folS in (follicleS['follicle']):
            ...
            follicleGrpOff.append(follicleGrpOffset)
            ...
            follicleCtrl = ct.Control(prefix=au.prefix_name(folS), groups_ctrl=['Zro', 'Twist', 'Offset'],
                                      ctrl_color='lightPink', ctrl_size=scale * 0.65, shape=ctrlDetails)
        ...
        # Twist deformer: Sum the twist and the roll
            ...
            mc.connectAttr((self.ctrlDown.control + '.twist'), (self.sumTopPma + '.input1D[0]'))
            mc.connectAttr((self.ctrlDown.control + '.twistOffset'), (self.sumTopPma + '.input1D[1]'))
            ...
            mc.connectAttr((self.ctrlUp.control + '.twist'), (self.sumEndPma + '.input1D[0]'))
            mc.connectAttr((self.ctrlUp.control + '.twistOffset'), (self.sumEndPma + '.input1D[1]'))
        ...
        # Twist deformer: Set up the affect of the deformer
            ...
            mc.connectAttr((self.ctrlDown.control + '.affectToMid'), (topAffMdl + '.input2'))
            ...
            mc.connectAttr((self.ctrlUp.control + '.affectToMid'), (endAffMdl + '.input2'))
        ...
        # Create deformers: Blendshape
            mc.blendShape(rbnBlendshp, e=1, t=(geoPlane[0], 1, geoPlaneTwist[0], 1.0))
            mc.blendShape(rbnBlendshp, e=1, t=(geoPlane[0], 2, geoPlaneSine[0], 1.0))

            mc.blendShape(rbnBlendshp, e=True, w=[(1, 1.0), (2, 1.0)])
            ...
            for folV, folGrpOffst in zip(follicleV['follicle'], follicleGrpOff):
                # combine multiplier
        ...
        mc.parent(deformCrv, self.jointUpOrientParent[0],
        ...
        mc.aimConstraint(self.ctrlUp.control, self.ctrlDown.control, self.ctrlMid.parent_control[1], mo=1, w=1,
        ...
        au.lock_hide_attr(['t', 'r', 's'], self.grpNoTransformZro[0])
        ...
        if rotateCtrl:
            mc.blendShape(rbnBlendshp, e=1, t=(geoPlane[0], 3, geoPlaneOrient[0], 1.0))
            mc.blendShape(rbnBlendshp, e=True, w=[(3, 1.0)])

        else:
            mc.delete(geoPlaneOrient[0], sumDoubleCtrlDown, sumDoubleCtrlUp, self.jointUpOrientParent[0],
                      self.jointMidOrientParent[0], self.jointDownOrientParent[0] )
