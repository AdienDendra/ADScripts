...
class Build:
    ...
    # ==================================================================================================================
    ### CREATE CONTROL FK
        self.controller_upper_limb_fk   = ct.Control(match_obj_first_position=upper_limb_fk_jnt, prefix='%s' % prefix_upper_limb_fk, side=side,
                                                     shape=ct.CIRCLEPLUS, groups_ctrl=['Zro', 'Offset'], ctrl_size=scale,
                                                     ctrl_color='yellow', gimbal=True, lock_channels=['v', 's'],
                                                     connection=['parent'])

        self.controller_middle_limb_fk  = ct.Control(match_obj_first_position=middle_limb_fk_jnt, prefix='%s' % (prefix_middle_limb_fk), side=side,
                                                     shape=ct.CIRCLEPLUS, groups_ctrl=['Zro', 'Offset'], ctrl_size=scale,
                                                     ctrl_color='yellow', gimbal=True, lock_channels=['v', 's'],
                                                     connection=['parent'])

        self.controller_lower_limb_fk   = ct.Control(match_obj_first_position=lower_limb_fk_jnt, prefix='%s' % (prefix_lower_limb_fk), side=side,
                                                     shape=ct.CIRCLEPLUS, groups_ctrl=['Zro', 'Offset'], ctrl_size=scale,
                                                     ctrl_color='yellow', gimbal=True, lock_channels=['v', 's'],
                                                     connection=['parent'])
    ...
    # ==================================================================================================================
        ### CREATE CONTROL IK
        self.controller_upper_limb_ik = ct.Control(match_obj_first_position=upper_limb_ik_jnt, prefix='%s' % (prefix_upper_limb_ik), side=side, shape=ct.CUBE,
                                                   groups_ctrl=['Zro', 'Offset'], ctrl_size=scale * 0.65,
                                                   ctrl_color='red', gimbal=True, lock_channels=['v', 'r', 's'],
                                                   connection=['pointCons'])

        self.controller_pole_vector_ik = ct.Control(match_obj_first_position=pole_vector_ik_jnt, prefix='%s' % (prefix_pole_vector_ik), side=side, shape=ct.JOINT,
                                                    groups_ctrl=['Zro', 'Offset'], ctrl_size=scale * 0.25,
                                                    ctrl_color='red', gimbal=False, lock_channels=['v', 'r', 's'],
                                                    connection=['pointCons'])

        self.controller_lower_limb_ik = ct.Control(match_obj_first_position=lower_limb_ik_jnt, prefix='%s' % (prefix_lower_limb_ik), side=side, shape=ct.CUBE,
                                                   groups_ctrl=['Zro', 'Offset'], ctrl_size=scale * 0.65,
    ...
    # ==================================================================================================================
        ...
        self.end_limb_ik_hdl = mc.ikHandle(sj=lower_limb_ik_jnt, ee=end_limb_ik_jnt, sol='ikSCsolver', n='%s%s_hdl' % (prefix_end_limb_ik, side))

        poleVector_lower_constraint = mc.poleVectorConstraint(self.controller_pole_vector_ik.control, self.lower_limb_ik_hdl[0])
    ...
    # IK STRETCH, ELBOW SNAP, SLIDE JOINT AND SOFT IK SETUP
        ...
        mc.delete(mc.aimConstraint(self.pos_upper_limb_jnt, self.pos_lower_limb_jnt, o=(0, 0, 0), w=1.0, aim=(0, -1, 0), u=(-1, 0, 0),
                                   wut='vector', wu=(0, 1, 0)))
        mc.makeIdentity(self.pos_lower_limb_jnt, a=1, r=1, n=2, pn=1)

        # aiming the pos wrist joint to upper limb joint
        mc.delete(mc.aimConstraint(self.pos_lower_limb_jnt, self.pos_upper_limb_jnt, o=(0, 0, 0), w=1.0, aim=(0, 1, 0), u=(-1, 0, 0),
                                   wut='vector', wu=(0, 1, 0)))
        mc.makeIdentity(self.pos_upper_limb_jnt, a=1, r=1, n=2, pn=1)

        # softIk distance
        mc.select(cl=1)
        self.position_softIk_jnt = mc.duplicate(self.pos_lower_limb_jnt, name='%s%s%s_jnt' % (prefix, 'SoftIkDist', side))[0]
        ...
        distance_main_ik = self.limb_distance(prefix=prefix, pos_up_jnt=self.pos_upper_limb_jnt, pos_low_jnt=self.position_limb_jnt,
                                              dist_between_name='CtrlIkStretch', side=side)

        # create distance node from lower limb to soft ik jnt
        distance_lower_softIk = self.limb_distance(prefix=prefix, pos_up_jnt=self.pos_lower_limb_jnt, pos_low_jnt=self.position_softIk_jnt,
                                                   dist_between_name='SoftIkStretch', side=side)

        # create distance node from upper limb to pole vector jnt
        distance_upper_poleVector = self.limb_distance(prefix=prefix_upper_limb_ik, pos_up_jnt=self.pos_upper_limb_jnt, pos_low_jnt=self.pos_pole_vector_jnt,
                                                       dist_between_name='Snap', side=side)

        # create distance node from pole vector to soft ik jnt
        distance_poleVector_softIk = self.limb_distance(prefix=prefix_middle_limb_ik, pos_up_jnt=self.pos_pole_vector_jnt, pos_low_jnt=self.position_softIk_jnt,
                                                        dist_between_name='Snap', side=side)
    ...
    ## CREATE GENERAL SCALE IK NODES
        ...
        self.scale_decompose = mc.shadingNode('decomposeMatrix', asUtility=1, n='%s%s%s_dmtx' % (prefix, 'SoftSlideIkScale', side))
        self.scale_soft_slide_mdn = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'SoftSlideIkScale', side))
        ...
        self.scale_stretch_slide_mdn = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'StretchIkScale', side))
        ...
        self.scale_poleVector_snap_middle_mdn = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'SnapUrIkScale', side))
        ...
        self.scale_poleVector_snap_lower_mdn = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'SnapLrIkScale', side))
    ...
    ## SOFT IK SETUP
        ...
        soft_ik_subtract_scale = mc.shadingNode('plusMinusAverage', asUtility=1, n='%s%s%s_pma' % (prefix, 'SoftIkSubtScale', side))
        ...
        # multiply by the 10
        soft_ik_multiply_dist = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'SoftIkMulDist', side))
        ...
        soft_ik_dist_multiply_scale = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'SoftIkMulScale', side))
        ...
        soft_ik_distance_divide = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'SoftIkMulSubtDist', side))
        ...
        soft_ik_subtract_pma = mc.shadingNode('plusMinusAverage', asUtility=1, n='%s%s%s_pma' % (prefix, 'SoftIkSubtLength', side))
        ...
        soft_ik_distance_pma = mc.shadingNode('plusMinusAverage', asUtility=1, n='%s%s%s_pma' % (prefix, 'SoftIkSubtDist', side))
        ...
        soft_ik_add_adl = mc.shadingNode('addDoubleLinear', asUtility=1, n='%s%s%s_adl' % (prefix, 'SoftIkOffset', side))
        ...
        soft_ik_distance_div = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'SoftIkDivDist', side))
        ...
        soft_ik_multiply_reverse = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'SoftIkRev', side))
        ...
        soft_ik_multiply_exponent = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'SoftIkMulExp', side))
        ...
        soft_ik_pma_exponent = mc.shadingNode('plusMinusAverage', asUtility=1, n='%s%s%s_pma' % (prefix, 'SoftIkSubtExp', side))
        ...
        soft_ik_condition_exponent = mc.shadingNode('condition', asUtility=1, n='%s%s%s_cnd' % (prefix, 'SoftIkExp', side))
    ...
    ## STRETCH IK SETUP
        ...
        stretch_ik_multiplier = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s_mdn' % (prefix, 'StretchIkMulRev', side))
        ...
        stretch_ik_upper_sum = self.limb_stretch_ik(obj_poleVector='StretchIkUr', prefix=prefix, side=side, div_value=div_mid_value,
                                                    stretch_ik_multiplier=stretch_ik_multiplier, get_value_tx_upper_limb_jnt=get_value_tx_upper_limb_jnt,
                                                    length_limb=length_upper_limb)

        stretch_ik_mid_sum = self.limb_stretch_ik(obj_poleVector='StretchIkMd', prefix=prefix, side=side, div_value=div_lower_value,
    ...
    ## SLIDE MIDDLE LIMB IK SETUP
        ...
        mc.connectAttr(self.controller_lower_limb_ik.control + '.%s%s' % (prefix, 'Slide'), slide_ik_set_range + '.valueX')
        ...
        slide_ik_pma_difference_jnt = mc.shadingNode('plusMinusAverage', asUtility=1, n='%s%s%s_pma' % (prefix, 'SlideIkSubtrJntDist', side))
        ...
        slide_ik_mid_multiply_set_range = self.limb_slide_setRange_ik(prefix=prefix, side=side, obj_poleVector='SlideIkMd',
                                                                      slide_ik_pma_difference_jnt=slide_ik_pma_difference_jnt,
                                                                      div_value=div_mid_value, slide_ik_setRange=slide_ik_set_range)

        slide_ik_lower_multiply_set_range = self.limb_slide_setRange_ik(prefix=prefix, side=side, obj_poleVector='SlideIkLr',
                                                                        slide_ik_pma_difference_jnt=slide_ik_pma_difference_jnt,
                                                                        div_value=div_lower_value, slide_ik_setRange=slide_ik_set_range)

        # create condition for both middle limb and wrist sliding
        slide_ik_mid_lower_condition = mc.shadingNode('condition', asUtility=1, n='%s%s%s_cnd' % (prefix, 'SlideIk', side))
        ...
        slide_ik_mid_pma_stretch = self.limb_slide_pma_stretch_ik(obj_poleVector='SlideIkMd', prefix=prefix, side=side,
                                                                  get_value_tx_upper_limb_jnt=get_value_tx_upper_limb_jnt,
                                                                  operation_one=1, operation_two=2, stretch_ik_sum=stretch_ik_upper_sum,
                                                                  slide_ik_md_lower_condition=slide_ik_mid_lower_condition)

        slide_ik_lower_pma_stretch = self.limb_slide_pma_stretch_ik(obj_poleVector='SlideIkLr', prefix=prefix, side=side,
                                                                    get_value_tx_upper_limb_jnt=get_value_tx_upper_limb_jnt,
                                                                    operation_one=2, operation_two=1, stretch_ik_sum=stretch_ik_mid_sum,
                                                                    slide_ik_md_lower_condition=slide_ik_mid_lower_condition)

    ## SLIDE MIDDLE LIMB COMBINE TO STRETCH ON/OFF
        # create pma for sum to total value joint - distance
        slide_ik_pma_sum_difference_jnt = mc.shadingNode('plusMinusAverage', asUtility=1, n='%s%s%s_pma' % (prefix, 'SlideIkSumJntDist', side))
    ...
    ## SNAP IK SETUP
        ...
        # snap upper limb
        self.limb_poleVector_snap_ik(obj_poleVector='Md', get_value_tx_upper_limb_jnt=get_value_tx_upper_limb_jnt, prefix=prefix, side=side,
                                     prefix_pole_vector_ik=prefix_pole_vector_ik, slide_ik_pma_stretch=slide_ik_mid_pma_stretch, limb_ik_jnt=middle_limb_ik_jnt,
                                     scale_poleVector_snap_mdn=self.scale_poleVector_snap_middle_mdn)

        # snap middle limb
        self.limb_poleVector_snap_ik(obj_poleVector='Lr', get_value_tx_upper_limb_jnt=get_value_tx_upper_limb_jnt, prefix=prefix, side=side,
                                     prefix_pole_vector_ik=prefix_pole_vector_ik, slide_ik_pma_stretch=slide_ik_lower_pma_stretch,
    ...
    ## CREATE CURVE FOR POLE VECTOR CONTROLLER
        self.curve_poleVector_ik = mc.curve(d=1, p=[(0, 0, 0), (0, 0, 0)], k=[0, 1], n=('%s%s_crv' % (prefix_pole_vector_ik, side)))
        self.clusterBase, self.clusterBaseHdl =mc.cluster('%s.cv[0]' % self.curve_poleVector_ik, n='%s01%s_cls' % (prefix_pole_vector_ik, side),
                                                          wn=(middle_limb_ik_jnt, middle_limb_ik_jnt))
        self.clusterPoleVector, self.clusterPoleVectorHdl =mc.cluster('%s.cv[1]' % self.curve_poleVector_ik, n='%s02%s_cls' % (prefix_pole_vector_ik, side),
                                                                      wn=(self.controller_pole_vector_ik.control, self.controller_pole_vector_ik.control))
    ...
    # ==================================================================================================================
        if arm:
            # FK
            self.shoulder_fk = self.limb_follow_fk(prefix_upper_limb_fk=prefix_upper_limb_fk, upper_limb_fk_jnt=upper_limb_fk_jnt,
                                                   locator_name='Shoulder', prefix=prefix, side=side, second_term=0)

            shoulder_fk_point_constraint = mc.pointConstraint(self.shoulder_fk, self.controller_upper_limb_fk.parent_control[0],
                                                              mo=1)

            # hip
            self.hip_fk = self.limb_follow_fk(prefix_upper_limb_fk=prefix_upper_limb_fk, upper_limb_fk_jnt=upper_limb_fk_jnt,
                                              locator_name='Hip', prefix=prefix, side=side, second_term=1)
            # World
            self.world_fk = self.limb_follow_fk(prefix_upper_limb_fk=prefix_upper_limb_fk, upper_limb_fk_jnt=upper_limb_fk_jnt,
                                                locator_name='World', prefix=prefix, side=side, second_term=2)

            # IK
            # shoulder
            self.shoulder_ik = self.limb_follow_ik(prefix_upper_limb_ik=prefix_upper_limb_ik, lower_limb_ik_jnt=lower_limb_ik_jnt,
                                                   locator_name='Shoulder', prefix=prefix, side=side, second_term=0)
            # hip
            self.hip_ik = self.limb_follow_ik(prefix_upper_limb_ik=prefix_upper_limb_ik, lower_limb_ik_jnt=lower_limb_ik_jnt,
                                              locator_name='Hip', prefix=prefix, side=side, second_term=1)
            # World
            self.world_ik = self.limb_follow_ik(prefix_upper_limb_ik=prefix_upper_limb_ik, lower_limb_ik_jnt=lower_limb_ik_jnt,
                                                locator_name='World', prefix=prefix, side=side, second_term=2)
            # rename constraint
            ...
            hip_fk_point_constraint = mc.pointConstraint(self.hip_fk, self.controller_upper_limb_fk.parent_control[0], mo=1)
    ...
    # ==================================================================================================================
        ...
        self.controller_FkIk_limb_setup = ct.Control(match_obj_first_position=lower_limb_fk_jnt, prefix='%s' % (prefix_limb_setup), side=side,
        ...
        au.connect_part_object(obj_base_connection='FkIk', target_connection='inputX', obj_name=self.controller_FkIk_limb_setup.control,
                               target_name=[self.limb_setup_reverse], select_obj=False)

        au.connect_part_object(obj_base_connection='outputX', target_connection='visibility', obj_name=self.limb_setup_reverse,
                               target_name=[self.controller_upper_limb_fk.parent_control[0]], select_obj=False)

        # set on/off attribute IK
        au.connect_part_object(obj_base_connection='FkIk', target_connection='visibility', obj_name=self.controller_FkIk_limb_setup.control,
    ...
    # # ==================================================================================================================
    # #                                              ADDITIONAL JOINT ATTRIBUTE
    # # ==================================================================================================================
    #     if baseTipShapeJoint or upperLimbShapeJoint or poleVecLimbShapeJoint or lowerLimbShapeJoint:
    #         au.addAttribute(objects=[self.controllerFKIKLimbSetup.control], longName=['addJoint'],
    #                         niceName=[' '], at="enum",
    #                         en='Add Joint', cb=True)
    #     if baseTipShapeJoint:
    #         au.addAttribute(objects=[self.controllerFKIKLimbSetup.control], longName=[prefixBaseOrTipLimb + 'Shape'],
    #                         attributeType="float", min=0, max=1, dv=0.5, cb=True)
    #     if upperLimbShapeJoint:
    #         au.addAttribute(objects=[self.controllerFKIKLimbSetup.control], longName=[prefixUpperLimb + 'Shape'],
    #                         attributeType="float", min=0, max=1, dv=0.5, cb=True)
    #     if poleVecLimbShapeJoint:
    #         au.addAttribute(objects=[self.controllerFKIKLimbSetup.control], longName=[prefixPoleVecLimb + 'Shape'],
    #                         attributeType="float", min=0, max=1, dv=0.5, cb=True)
    #     if lowerLimbShapeJoint:
    #         au.addAttribute(objects=[self.controllerFKIKLimbSetup.control], longName=[prefixLowerLimb + 'Shape'],
    #                         attributeType="float", min=0, max=1, dv=0.5, cb=True)
    ...
    # ==================================================================================================================
        ## MID COMBINE DETAIL
        # create controller
        self.ctrl_mid_middle_limb = ct.Control(match_obj_first_position=middle_limb_jnt, prefix=prefix + 'DtlCombine', side=side, groups_ctrl=['Zro'],
        ...
        # ROLL
        self.adl_upper_limb_combine = mc.createNode('addDoubleLinear', n=(prefix_upper_limb_dtl + 'RollCombine' + side + '_adl'))
        mc.connectAttr(self.ctrl_mid_middle_limb.control + '.roll', self.adl_upper_limb_combine + '.input2')

        self.adl_middle_limb_combine  = mc.createNode('addDoubleLinear', n=(prefix_middle_limb_dtl + 'RollCombine' + side + '_adl'))
        ...
        if detail_limb_deformer:
            ...
            ## VOLUME POSITION
            ## upper limb
            self.volume_pos_mdl_upper_limb = self.limb_volume_position(prefix_limb_dtl=prefix_upper_limb_dtl, side=side, operation=2, num_joint=number_joints,
                                                                       dv_min=0, dv_max=0.5, v_min=1, v_max=0)

            ## middle limb
            self.volume_pos_mdl_middle_limb = self.limb_volume_position(prefix_limb_dtl=prefix_middle_limb_dtl, side=side, operation=4, num_joint=number_joints,
                                                                        dv_min=-0.5, dv_max=0, v_min=0, v_max=1)

        # Extra attributes
        au.add_attribute(objects=[self.ctrl_mid_middle_limb.control], long_name=['extraSep'], nice_name=[' '], at="enum",
                         en='Extra',
                         channel_box=True)
        au.add_attribute(objects=[self.ctrl_mid_middle_limb.control], long_name=['detailBaseCtrlVis'], at="long", min=0, max=1, dv=0,
                         channel_box=True)
    ...
    def limb_follow_fk(self, prefix_upper_limb_fk, upper_limb_fk_jnt, locator_name, prefix, side, second_term):
        ...
        # create condition
        part_limb_fol_fk_cnd = mc.shadingNode('condition', asUtility=1, n='%s%s%s_cnd' % (prefix, locator_name + 'FkFollow',
                                                                                          side))
    ...
    def limb_follow_ik(self, prefix_upper_limb_ik, lower_limb_ik_jnt, locator_name, prefix, side, second_term):
        ...
        # create condition
        part_limb_fol_ik_condition = mc.shadingNode('condition', asUtility=1, n='%s%s%s_cnd' % (prefix, locator_name + 'IkFollow', side))
        ...
        mc.connectAttr(part_limb_fol_ik_condition + '.outColorR', ('%s.%sW%s' % (self.blend_limb_ik_parent[0], locator[0], second_term)))
    ...
    def limb_switch_FkIk(self, limb_fk_jnt, limb_ik_jnt, limb_jnt):
        ...
        au.connect_part_object(obj_base_connection='outputX', target_connection='%sW0' % limb_fk_jnt, obj_name=self.limb_setup_reverse,
    ...
    def limb_stretch_ik(self, obj_poleVector, prefix, side, div_value, stretch_ik_multiplier, get_value_tx_upper_limb_jnt, length_limb):
        # limb
        stretch_ik_mul_distance = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s%s_mdn' % (prefix, obj_poleVector, 'MulDist', side))
        ...
        stretch_ik_mul_set = mc.shadingNode('multiplyDivide', asUtility=1, n='%s%s%s%s_mdn' % (prefix, obj_poleVector, 'MulSet', side))
        ...
        stretch_ik_sum = mc.shadingNode('plusMinusAverage', asUtility=1, n='%s%s%s%s_pma' % (prefix, obj_poleVector, 'Sum', side))
        if get_value_tx_upper_limb_jnt > 0:
    ...
    def limb_slide_setRange_ik(self, prefix, side, obj_poleVector, slide_ik_pma_difference_jnt, div_value, slide_ik_setRange):
    ...
    def limb_slide_pma_stretch_ik(self, obj_poleVector, prefix, side, get_value_tx_upper_limb_jnt, operation_one, operation_two, stretch_ik_sum, slide_ik_md_lower_condition):
    ...
    def limb_slide_combine_ik(self, obj_poleVector, prefix, side, slide_ik_pma_sum_difference_jnt, div_value, distance_main_ik_value,
        ...
        slide_ik_dist_condition = mc.shadingNode('condition', asUtility=1, n='%s%s%s%s_cnd' % (prefix, obj_poleVector, 'Dist', side))
    ...
    def limb_poleVector_snap_ik(self, get_value_tx_upper_limb_jnt, prefix, side, obj_poleVector, prefix_pole_vector_ik, slide_ik_pma_stretch,
        ...
        poleVector_mult_rev = mc.shadingNode('multDoubleLinear', asUtility=1,
                                             n='%s%s%s%s%s_mdl' % (prefix, 'PoleVecSnapIk', obj_poleVector, 'Rev', side))
        if get_value_tx_upper_limb_jnt > 0:
