...
class CreateDetail:
    ...
    ### CREATING JOINT TEMPORARY FOR SPLITTING JNT FOLLICLE
        ...
        if detail_spine_deformer:
            ...
            mc.connectAttr((volume_multiply_position_mdl + '.output'), (self.squash_deformer[1] + vol_input_mdn[parallel_axis][0]))
        ...
        skin_cluster = mc.skinCluster([self.joint_skin_up[0], self.joint_skin_mid[0], self.joint_skin_down[0]], geo_plane_skin,
        ...
        mc.skinPercent(skin_cluster[0], skin_percent7, tv=[(self.joint_skin_down[0], 0.9), (self.joint_skin_mid[0], 0.1)])
        mc.skinPercent(skin_cluster[0], skin_percent1, tv=[(self.joint_skin_down[0], 0.7), (self.joint_skin_mid[0], 0.3)])
        mc.skinPercent(skin_cluster[0], skin_percent2, tv=[(self.joint_skin_down[0], 0.4), (self.joint_skin_mid[0], 0.6)])
        ...
        skin_chest_constraint = mc.parentConstraint(list_spine_ctrl_fk[2], list_spine_ctrl_ik[1], self.joint_skin_down[0], mo=0)
        ...
        skin_mid_constraint = mc.parentConstraint(list_spine_ctrl_fk[1], list_spine_ctrl_ik[0], self.joint_skin_mid[0], mo=1)
        ...
        for fol_s, fol_shp, jntSpine in zip(follicle_s['follicle'], follicle_s['folShape'], list_spine_jnt[1:4]):
            ...
            follicleCtrl = ct.Control(prefix=au.prefix_name(fol_s), groups_ctrl=['Zro'], side=side, ctrl_color='lightPink',
    ...
    # ==================================================================================================================
        if detail_spine_deformer:
            ...
            for fol_s, fol_shp, fol_v, fol_grp_offset, fol_ctrl, in zip (follicle_s['follicle'], follicle_s['folShape'],
                                                                         follicle_v['follicle'], follicle_grp_offset, follicle_ctrl):
                # Parenting the follicles to grp follicleS
                mc.parent(fol_v, self.grp_follicle_volume)

                # Multiplier from follicleS to multiplier for the size
                mult_mpd_size = mc.shadingNode('multiplyDivide', asUtility=1, name=au.prefix_name(fol_s) + 'MultiplierSize_mdn')
        ...
        for obj in objects:
            ...
            for x in range(0, len(long_name)):
                ...
                if not mc.attributeQuery(long_name[x], node=obj, exists=True):
                    ...
                    mc.setAttr((obj + '.' + long_name[x]), k=keyable, e=1, cb=channel_box) if separator else mc.setAttr((obj + '.' + long_name[x]), k=keyable, e=1, cb=channel_box)
