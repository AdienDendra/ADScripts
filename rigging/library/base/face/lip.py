...
class Build:
    def __init__(self, curve_lip_template,
        ...
        mc.parent(self.sticky_curve_grp, self.sticky_cluster_hdl_grp, self.origin_locator_grp, self.mid_locator_grp,
                  self.sticky_grp)
    ...
    def controller_lip(self, scale, side_LFT, side_RGT, controller_lip_low, ctrl_color, suffix_controller):
        ...
        # CONTROLLER MID
        self.controller_bind_mid = ct.Control(match_obj_first_position=self.jnt_mid,
        ...
        # CONTROLLER RGT 01
        self.controller_bind01_RGT = ct.Control(match_obj_first_position=self.jnt01_RGT,
                                                prefix=self.prefix_name_curve + 'Drv01',
                                                shape=ct.SQUAREPLUS, groups_ctrl=['Zro', 'Offset', 'All'],
                                                ctrl_size=scale * 0.05,
                                                ctrl_color=ctrl_color, lock_channels=['v', 's'], side=side_RGT,
                                                suffix=suffix_controller
                                                )

        # CONTROLLER RGT 02
        self.controller_bind02_RGT = ct.Control(match_obj_first_position=self.jnt02_RGT,
                                                prefix=self.prefix_name_curve + 'Drv02',
                                                shape=ct.SQUAREPLUS, groups_ctrl=['Zro', 'Offset'],
                                                ctrl_size=scale * 0.07,
                                                ctrl_color=ctrl_color, lock_channels=['v', 's'], side=side_RGT,
                                                suffix=suffix_controller
                                                )
        # CONTROLLER LFT 01
        self.controller_bind01_LFT = ct.Control(match_obj_first_position=self.jnt01_LFT,
                                                prefix=self.prefix_name_curve + 'Drv01',
                                                shape=ct.SQUAREPLUS, groups_ctrl=['Zro', 'Offset', 'All'],
                                                ctrl_size=scale * 0.05,
                                                ctrl_color=ctrl_color, lock_channels=['v', 's'], side=side_LFT,
                                                suffix=suffix_controller
                                                )
        # CONTROLLER LFT 02
        self.controller_bind02_LFT = ct.Control(match_obj_first_position=self.jnt02_LFT,
                                                prefix=self.prefix_name_curve + 'Drv02',
                                                shape=ct.SQUAREPLUS, groups_ctrl=['Zro', 'Offset'],
                                                ctrl_size=scale * 0.07,
                                                ctrl_color=ctrl_color, lock_channels=['v', 's'], side=side_LFT,
        ...
        mc.parent(self.controller_bind_mid.parent_control[0], self.controller_bind01_RGT.parent_control[0],
        ...
        if controller_lip_low:
            ...
            tf.bind_rotate_reverse(control=self.controller_bind02_RGT.control,
                                   input_2X=-1, input_2Y=-1, input_2Z=1,
            ...
            tf.bind_rotate_reverse(control=self.controller_bind01_RGT.control,
                                   input_2X=-1, input_2Y=-1, input_2Z=1,
            ...
            tf.bind_rotate_reverse(control=self.controller_bind02_LFT.control,
                                   input_2X=-1, input_2Y=1, input_2Z=-1,
            ...
            tf.bind_rotate_reverse(control=self.controller_bind01_LFT.control,
                                   input_2X=-1, input_2Y=1, input_2Z=-1,
    ...
    def sticky_lip(self, lip_curve, scale):
        ...
        self.bind_sticky_mid_crv = mc.duplicate(self.deform_curve, n=self.prefix_name_curve + 'BindStickyMid' + '_crv')[
            0]
        ...
        sticky_mid_wire_deformer[0] = mc.rename(sticky_mid_wire_deformer[0],
        ...
        sticky_origin_wire_deformer[0] = mc.rename(sticky_origin_wire_deformer[0],
        ...
        mc.blendShape(self.sticky_move_crv, lip_curve, n=(self.prefix_name_curve + 'StickyMove' + '_bsn'),
                      weight=[(0, 1)])
    ...
    def duplicate_locator_and_add_pci_node(self, name_locator, curve):
        ...
        sticky_locator_grp = mc.createNode('transform',
    ...
    def wire_bind_curve(self, lip_curve, offset_jnt02_bind_position, lip01_direction, lip02_direction,
        ...
        # CREATE BIND CURVE
        deform_curve = mc.duplicate(lip_curve)[0]
        ...
        skin_cluster = mc.skinCluster([jnt01_LFT, jnt02_LFT, jnt01_RGT, jnt02_RGT, jnt_mid], deform_curve,
                                      n='%s%s%s' % ('wire', self.prefix_name_curve.capitalize(), '_sc'), tsb=True, bm=0,
                                      sm=0, nw=1, mi=3)
        ...
        mc.skinPercent(skin_cluster[0], skin_percent_index1, tv=[(jnt01_RGT, 0.80), (jnt02_RGT, 0.20)])
        mc.skinPercent(skin_cluster[0], skin_percent_index2, tv=[(jnt01_RGT, 0.50), (jnt02_RGT, 0.50)])
        mc.skinPercent(skin_cluster[0], skin_percent_index3, tv=[(jnt02_RGT, 1.0)])
        mc.skinPercent(skin_cluster[0], skin_percent_index4, tv=[(jnt02_RGT, 0.50), (jnt_mid, 0.50)])
        mc.skinPercent(skin_cluster[0], skin_percent_index5, tv=[(jnt_mid, 1.0)])
        mc.skinPercent(skin_cluster[0], skin_percent_index6, tv=[(jnt02_LFT, 0.50), (jnt_mid, 0.50)])
        mc.skinPercent(skin_cluster[0], skin_percent_index7, tv=[(jnt02_LFT, 1.0)])
        mc.skinPercent(skin_cluster[0], skin_percent_index8, tv=[(jnt01_LFT, 0.50), (jnt02_LFT, 0.50)])
        mc.skinPercent(skin_cluster[0], skin_percent_index9, tv=[(jnt01_LFT, 0.80), (jnt02_LFT, 0.20)])
        mc.skinPercent(skin_cluster[0], skin_percent_index10, tv=[(jnt01_LFT, 1.0)])
    ...
    def create_reset_mouth_position_grp(self, mouth_base_jnt):
        ...
        self.mouth_ctrl_grp_offset = mc.group(em=1, n=self.prefix_name_curve + 'DrvAllCtrlMouthOffset_grp',
                                              p=self.mouth_ctrl_grp)
        mc.xform(self.mouth_ctrl_grp, ws=1, t=position_mouth)

        # RESETTING TRANSFORM FOR SET
        self.reset_all_mouth_ctrl_grp = mc.createNode('transform',
                                                      n=self.prefix_name_curve + 'DrvAllResetUtilMouth_grp')
        self.reset_all_mouth_ctrl_grp_offset = mc.group(em=1,
    ...
    def create_joint_lip(self, curve, scale, ctrl_color, suffix_controller, game_bind_joint):
        ...
        for index, object in enumerate(self.curve_vertex):
            ...
            control_group = ct.Control(match_obj_first_position=self.joint,
