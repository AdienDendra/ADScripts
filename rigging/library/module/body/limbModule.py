...
from rigging.library.base.body import limbPart as rlbb_limbPart, limb as rlbb_limb
from rigging.library.module import baseModule as rlm_baseModule, tmpSingleModule as rlm_tmpSingleModule
from rigging.library.module.body import handModule as rlmb_handModule, footModule as rlmb_footModule
from rigging.library.utils import controller as rlu_controller, softIkSetup as rlu_softIkSetup, \
    addAttrMessage as rlu_addAttrMessage, poleVector as rlu_poleVector
...
class Limb:
    def __init__(self,
        ...
        if singleModule:
            if arm:
                if left_side:
                    if cmds.objExists('upArmLFT_skn'):
                        cmds.error('please clean up the left arm first!')
                    else:
                        ...
                        self.single_module_arm(base_controller=base_controller, upper_limb_jnt=self.sj.upArm_LFT,
                                               ...
                                               side_RGT=side_RGT)
                        if end_limb:
                            rlmb_handModule.Hand(parent=True, arm_object=self.part_control_grp,
                                                 ...
                                                 middle_finger_up=self.sj.middle2_LFT,
                        ...
                        cmds.delete(self.sIk.hand_LFT, self.sIk.palm_LFT, self.sIk.thumb1_LFT,
                        ...
                        cmds.delete(self.sj.hand_LFT, self.sj.palm_LFT, self.sj.thumb1_LFT)
                    ...
                    if cmds.objExists('upArmRGT_skn'):
                        cmds.error('please clean up the right arm first!')
                    else:
                        self.joint_driver(arm=True)
                        ...
                        # else:
                        cmds.delete(self.sIk.hand_RGT, self.sIk.palm_RGT, self.sIk.thumb1_RGT,
                        ...
                        cmds.delete(self.sj.hand_RGT, self.sj.palm_RGT, self.sj.thumb1_RGT)
                        cmds.delete(self.sj.root, self.ss.root, self.sFk.root, self.sIk.root, self.sTwistHelp.root,
                                    self.sAdd.root,
                                    )
            else:
                if left_side:
                    if cmds.objExists('upLegLFT_skn'):
                        cmds.error('please clean up the left leg first!')
                    else:
                        ...
                        legLFT = self.single_module_leg(base_controller, upper_limb_jnt=self.sj.upLeg_LFT,
                                                        ...
                                                        side_RGT=side_RGT)
                        if end_limb:
                            cmds.delete(legLFT[0], legLFT[1])
                            rlmb_footModule.Foot(leg=True, foot=True, upper_limb_jnt=self.sj.upLeg_LFT,
                                                 ...
                                                 lower_gimbal_fk_ctrl=self.lower_limb_fk_gimbal,
                                                 lower_limb_ik_hdl=self.lower_limb_ik_hdl,
                                                 end_limb_ik_hdl=self.end_limb_ik_hdl,
                                                 controller_FkIk_limb_setup=self.FkIk_limb_setup_controller,
                            ...
                            cmds.delete(self.ss.heel_LFT, self.ss.toe_LFT, self.ss.footIn_LFT, self.ss.footOut_LFT,
                    ...
                    if cmds.objExists('upLegRGT_skn'):
                        cmds.error('please clean up the right leg first!')
                    else:
                        self.joint_driver(arm=False)
                        legRGT = self.single_module_leg(base_controller, upper_limb_jnt=self.sj.upLeg_RGT,
                                                        ...
                                                        side_RGT=side_RGT)
                        if end_limb:
                            cmds.delete(legRGT[0], legRGT[1])
                            rlmb_footModule.Foot(leg=True, foot=True, upper_limb_jnt=self.sj.upLeg_RGT,
                            ...
                            cmds.delete(self.ss.heel_RGT, self.ss.toe_RGT, self.ss.footIn_RGT, self.ss.footOut_RGT,
            ...
            if limb:
                ...
                self.run_soft_ik = rlu_softIkSetup.run_soft_ik_joint(prefix=prefix, side=side,
                                                                     lower_limb_ik_gimbal=self.lower_limb_ik_gimbal,
    ...
    def module(self, arm, prefix, base_controller, prefix_upper_limb, prefix_upper_limb_fk, prefix_middle_limb_fk,
        ...
        getValueTxLimbJnt = cmds.xform(upper_limb_ik_jnt, ws=1, q=1, t=1)[0]

        ### GENERAL LIMB SETUP
        # limb position
        # create limb pole vector position
        ikh_vect_pos = cmds.ikHandle(sj=upper_limb_ik_jnt, ee=lower_limb_ik_jnt, sol='ikRPsolver')
        loc_poleVector_position = rlu_poleVector.create_poleVec_locator(ikh_vect_pos[0], constraint=False,
                                                                        length=4 * size)

        # ==============================================================================================================
        #                                               IMPORT LIMB MODULE
        # ==============================================================================================================
        build_limb = rlbb_limb.Build(
            arm=arm,
            prefix=prefix,
        ...
        cmds.delete(loc_poleVector_position)
        ...
        message = rlu_addAttrMessage.MessageAttribute(fkik_ctrl=self.FkIk_limb_setup_controller)
        ...
        if arm:
            ...
            cmds.parent(self.shoulder_fk_locator, clav_jnt)
            cmds.parent(self.hip_fk_locator, root_gimbal_ctrl)
            cmds.parent(self.world_fk_locator, part.utils_grp)

            # constraining follow Ik
            pac_clavicle_constraint = rt_utils.parent_constraint(clav_jnt, self.upper_limb_ik_controller_grp_zro)

            cmds.parent(self.shoulder_ik_locator, clav_jnt)
            cmds.parent(self.hip_ik_locator, root_gimbal_ctrl)
            cmds.parent(self.world_ik_locator, part.utils_grp)

            # rename constraint
            rt_utils.constraint_rename(pac_clavicle_constraint)
            ...
            ### FOLLOW ORIENTATION
            # constraining follow Fk
            cmds.parent(self.hip_fk_locator, pelvis_gimbal_ctrl)
            cmds.parent(self.world_fk_locator, part.utils_grp)

            # constraining follow Ik
            pac_pelvis_constraint = rt_utils.parent_constraint(pelvis_gimbal_ctrl,
                                                               self.upper_limb_ik_controller_grp_zro)

            cmds.parent(self.hip_ik_locator, pelvis_gimbal_ctrl)
            cmds.parent(self.world_ik_locator, part.utils_grp)

            # parent scale joint to joint driver
            cmds.parent(lower_limb_scale_jnt, lower_limb_jnt)
            # connect scale
            self.foot_scale(self.FkIk_limb_setup_controller, lower_limb_scale_jnt)

            # connect attribute translate and rotate
            cmds.connectAttr(end_limb_jnt + '.translate', end_limb_scale_jnt + '.translate')
            cmds.connectAttr(end_limb_jnt + '.rotate', end_limb_scale_jnt + '.rotate')

            # set to world
            cmds.setAttr(self.lower_limb_ik_control + '.follow', 1)

            # rename constraint
            rt_utils.constraint_rename(pac_pelvis_constraint)
        ...
        # controller
        cmds.parent(self.FkIk_limb_setup_controller_grp_zro, part.control_grp)
        cmds.parent(build_limb.ctrl_mid_middle_limb.parent_control[0], part.control_grp)

        # joint
        self.part_joint_grp = part.joint_grp
        cmds.parent(build_limb.pos_upper_limb_jnt, part.joint_grp)
        cmds.parent(build_limb.position_softIk_jnt, part.joint_grp)

        # non-trans
        cmds.parent(build_limb.curve_poleVector_ik, part.non_transform_grp)

        ### FK SETUP
        # parent lower to middle limb control group
        cmds.parent(self.lower_limb_fk_controller_grp_zro, self.middle_limb_fk_gimbal)

        # parent middle limb to upper limb control group
        cmds.parent(self.middle_limb_fk_controller_grp_zro, self.upper_limb_fk_gimbal)

        # parent all limb to part control group
        cmds.parent(self.upper_limb_fk_controller_grp_zro, part.control_grp)

        # connect scale module to decompose matrix
        cmds.connectAttr(base_controller.anim_control + '.worldMatrix[0]', build_limb.scale_decompose + '.inputMatrix')

        # stretch FK middle limb
        cmds.connectAttr(build_limb.upper_stretch_limb_fk + '.output',
                         self.middle_limb_fk_controller_grp_zro + '.translateY')

        # stretch FK wrist
        cmds.connectAttr(build_limb.middle_stretch_limb_fk + '.output',
                         self.lower_limb_fk_controller_grp_zro + '.translateY')

        ### IK SETUP
        # parent joint IK driver to joint grp
        cmds.parent(upper_limb_ik_jnt, part.joint_grp)

        # parent upper limb to part control group
        cmds.parent(self.upper_limb_ik_controller_grp_zro, part.control_grp)

        # parent elbow to part control group
        cmds.parent(self.poleVector_ik_controller_grp_zro, part.control_grp)

        # parent wrist to part control group
        cmds.parent(self.lower_limb_ik_controller_grp_zro, part.control_grp)

        # point constraint the stretching the limb
        pt_upper_limb_ik_constraint = cmds.pointConstraint(self.upper_limb_ik_gimbal, build_limb.pos_upper_limb_jnt,
                                                           mo=1)

        # aim constraining for stretching the limb
        aim_lower_limb_ik_constraint = cmds.aimConstraint(self.lower_limb_ik_gimbal, build_limb.pos_upper_limb_jnt,
                                                          mo=1,
                                                          aim=(0.0, 1.0, 0.0),
        ...
        elbow_follow_constraint = \
            cmds.parentConstraint(self.lower_limb_ik_control, world, self.poleVector_ik_controller_grp_zro, mo=1)[0]
        cmds.connectAttr(build_limb.controller_pole_vector_ik.control + '.follow',
                         elbow_follow_constraint + '.%sW0' % self.lower_limb_ik_control)

        elbow_cons_reverse = cmds.shadingNode('reverse', asUtility=1,
                                              n='%s%s%s_rev' % (prefix_pole_vector_ik, 'Follow', side))
        cmds.connectAttr(build_limb.controller_pole_vector_ik.control + '.follow', elbow_cons_reverse + '.inputX')
        cmds.connectAttr(elbow_cons_reverse + '.outputX', elbow_follow_constraint + '.%sW1' % world)

        # set ikRPsolver
        cmds.setAttr("ikRPsolver.tolerance", 1e-09)

        # constraining the controller Fk/Ik
        pac_lower_limb_constraint = cmds.parentConstraint(lower_limb_jnt, self.FkIk_limb_setup_controller_grp_zro, mo=1)
        ...
        #                                               UPPER LIMB DETAIL
        # ==============================================================================================================
        detail_upper_limb = rlbb_limbPart.CreateDetail(
        ...
        cmds.parent(detail_upper_limb.grp_no_transform_zro, part.non_transform_grp)
        cmds.parent(detail_upper_limb.grp_transform_zro, part.control_grp)

        # scale connect
        cmds.connectAttr(base_controller.scale_matrix_node + '.outputScale', detail_upper_limb.grp_transform + '.scale')

        # parent contraint detail jnt driver to grp offset middle limb
        pac_mid_dtl_upper_limb_constraint = cmds.parentConstraint(middle_limb_jnt,
                                                                  detail_upper_limb.ctrl_down.parent_control[1],
                                                                  mo=1)

        if arm:
            # parent constraint detail bind to grp transform detail
            pac_detail_limb_constraint = cmds.parentConstraint(clav_jnt, detail_upper_limb.grp_transform, mo=1)
            rt_utils.constraint_rename(pac_detail_limb_constraint)
        else:
            pac_detail_limb_constraint = cmds.parentConstraint(pelvis_gimbal_ctrl, detail_upper_limb.grp_transform,
                                                               mo=1)
            rt_utils.constraint_rename(pac_detail_limb_constraint)

        ### TWIST UPPER LIMB SETUP
        # create multi matrix
        mult_matrix_ctrl_up = cmds.shadingNode('multMatrix', asUtility=1,
                                               n='%s%s%s_mmtx' % (prefix_upper_limb_detail, 'CtrlUp', side))
        mult_matrix_ctrl_down = cmds.shadingNode('multMatrix', asUtility=1,
                                                 n='%s%s%s_mmtx' % (prefix_upper_limb_detail, 'CtrlDown', side))

        cmds.connectAttr(detail_upper_limb.ctrl_up.parent_control[2] + '.worldMatrix[0]',
                         mult_matrix_ctrl_up + '.matrixIn[0]')
        cmds.connectAttr(detail_upper_limb.ctrl_down.parent_control[0] + '.worldInverseMatrix[0]',
                         mult_matrix_ctrl_up + '.matrixIn[1]')

        cmds.connectAttr(detail_upper_limb.ctrl_down.parent_control[2] + '.worldMatrix[0]',
                         mult_matrix_ctrl_down + '.matrixIn[0]')
        cmds.connectAttr(detail_upper_limb.ctrl_up.parent_control[0] + '.worldInverseMatrix[0]',
                         mult_matrix_ctrl_down + '.matrixIn[1]')

        # create decompose matrix
        decompose_matrix_ctrl_up = cmds.shadingNode('decomposeMatrix', asUtility=1,
                                                    n='%s%s%s_dmtx' % (prefix_upper_limb_detail, 'CtrlUp', side))
        decompose_matrix_ctrl_down = cmds.shadingNode('decomposeMatrix', asUtility=1,
                                                      n='%s%s%s_dmtx' % (prefix_upper_limb_detail, 'CtrlDown', side))

        cmds.connectAttr(mult_matrix_ctrl_up + '.matrixSum', decompose_matrix_ctrl_up + '.inputMatrix')
        cmds.connectAttr(mult_matrix_ctrl_down + '.matrixSum', decompose_matrix_ctrl_down + '.inputMatrix')

        # create quat to euler
        quat_ctrl_up = cmds.shadingNode('quatToEuler', asUtility=1,
                                        n='%s%s%s_qte' % (prefix_upper_limb_detail, 'CtrlUp', side))
        quat_ctrl_down = cmds.shadingNode('quatToEuler', asUtility=1,
                                          n='%s%s%s_qte' % (prefix_upper_limb_detail, 'CtrlDown', side))

        # set rotation order to zxy
        cmds.setAttr(quat_ctrl_up + '.inputRotateOrder', 2)
        cmds.setAttr(quat_ctrl_down + '.inputRotateOrder', 2)

        cmds.connectAttr(decompose_matrix_ctrl_up + '.outputQuatY', quat_ctrl_up + '.inputQuatY')
        cmds.connectAttr(decompose_matrix_ctrl_up + '.outputQuatW', quat_ctrl_up + '.inputQuatW')

        cmds.connectAttr(decompose_matrix_ctrl_down + '.outputQuatY', quat_ctrl_down + '.inputQuatY')
        cmds.connectAttr(decompose_matrix_ctrl_down + '.outputQuatW', quat_ctrl_down + '.inputQuatW')
        ...
        for twist, offset in zip(self.set_grp_follicle_twist_upper_limb, self.set_grp_follicle_offset_upper_limb):
            reverseTwistNode = cmds.createNode('multDoubleLinear', n='%s_mdl' % (rt_utils.prefix_name(twist)))
            cmds.connectAttr(twist + '.rotateY', reverseTwistNode + '.input1')
            cmds.connectAttr(self.FkIk_limb_setup_controller + '.%s%s' % (prefix, 'MultTwist'),
                             reverseTwistNode + '.input2')
            cmds.connectAttr(reverseTwistNode + '.output', offset + '.rotateY')
        ...
        for i in range(len(self.set_grp_follicle_twist_upper_limb)):
            ...
            ctrl_up_mdl = cmds.shadingNode('multDoubleLinear', asUtility=1,
                                           n='%s%s%s%s_mdl' % (
                                               prefix_upper_limb_detail, str(i + 1).zfill(2), 'MultCtrlUp', side))

            cmds.setAttr(ctrl_up_mdl + '.input2', value)
            ctrl_up_mdl_node.append(ctrl_up_mdl)

            # ctrl down mult double linear
            ctrl_down_mdl = cmds.shadingNode('multDoubleLinear', asUtility=1,
                                             n='%s%s%s%s_mdl' % (
                                                 prefix_upper_limb_detail, str(i + 1).zfill(2), 'MultCtrlDown', side))
            cmds.setAttr(ctrl_down_mdl + '.input2', value)
            ctrl_down_mdl_node.append(ctrl_down_mdl)

            # connect quatCtrlDown and quatCtrlUp to respective multiply node
            cmds.connectAttr(quat_ctrl_up + '.outputRotateY', ctrl_up_mdl + '.input1')
            cmds.connectAttr(quat_ctrl_down + '.outputRotateY', ctrl_down_mdl + '.input1')

            # create matrix node for twist upper limb
            sum_up_down_matrix = cmds.shadingNode('plusMinusAverage', asUtility=1,
                                                  n='%s%s%s%s_pma' % (
        ...
        for up, down, pma, twist in zip(ctrl_up_mdl_node[::-1], ctrl_down_mdl_node, sum_up_down_matrix_node,
            ...
            cmds.connectAttr(up + '.output', pma + '.input1D[0]')
            cmds.connectAttr(down + '.output', pma + '.input1D[1]')
            cmds.connectAttr(pma + '.output1D', twist + '.rotateY')

        # ==============================================================================================================
        #                                        TWIST ROTATE UPPER LIMB
        # ==============================================================================================================
        # parent joint to joint grp
        cmds.parent(upper_limb_twist_help_driver_jnt, part.joint_grp)

        upper_limb_twist_help_joint = cmds.xform(upper_limb_twist_help_driver_jnt, q=1, ws=True, t=1)
        middle_limb_twist_help_joint = cmds.xform(middle_limb_twist_help_driver_jnt, q=1, ws=True, t=1)

        # create curve for ik spline
        curve_ik_spline_detail = cmds.curve(d=1, p=[upper_limb_twist_help_joint, middle_limb_twist_help_joint])
        self.limb_detail_hdl = cmds.ikHandle(sj=upper_limb_twist_help_driver_jnt, ee=middle_limb_twist_help_driver_jnt,
                                             sol='ikSplineSolver',
                                             n='%s%s_hdl' % (prefix_upper_limb + 'TwistHelp', side), ccv=False,
                                             c=curve_ik_spline_detail, ns=1, rootOnCurve=True)
        detail_hdl_curve = cmds.rename(curve_ik_spline_detail, '%s%s_crv' % (prefix_upper_limb + 'TwistHelp', side))

        # skin the spline curve
        cmds.skinCluster(upper_limb_jnt, middle_limb_jnt, detail_hdl_curve, tsb=True, mi=1)

        # parent to part grp
        grp_curve_ikh = cmds.group(empty=True, n=prefix_upper_limb + 'TwistHelp' + side + '_grp')
        cmds.parent(self.limb_detail_hdl[0], upper_limb_twist_help_driver_jnt, grp_curve_ikh)
        cmds.parent(grp_curve_ikh, detail_upper_limb.grp_transform)
        cmds.parent(detail_hdl_curve, part.non_transform_grp)

        # constraint twist help joint to zro controller up limb grp
        pac_detail_upper_limb_constraint = cmds.parentConstraint(upper_limb_twist_help_driver_jnt,
                                                                 detail_upper_limb.ctrl_up.parent_control[0])

        # hide the curve spline Ik
        cmds.hide(detail_hdl_curve, self.limb_detail_hdl[0], upper_limb_twist_help_driver_jnt,
                  middle_limb_twist_help_driver_jnt)

        # ==============================================================================================================
        #                                               MIDDLE LIMB DETAIL
        # ==============================================================================================================
        detail_lower_limb = rlbb_limbPart.CreateDetail(
        ...
        # parent main grp to still grp
        cmds.parent(detail_lower_limb.grp_no_transform_zro, part.non_transform_grp)
        cmds.parent(detail_lower_limb.grp_transform_zro, part.control_grp)

        # scale connect
        cmds.connectAttr(base_controller.scale_matrix_node + '.outputScale', detail_lower_limb.grp_transform + '.scale')

        # constraining middle limb bind dtl to middle limb grp transform
        pac_mid_limb_constraint = cmds.parentConstraint(middle_limb_jnt, detail_lower_limb.grp_transform, mo=1)

        # constraining wrist to middle limb down ctrl detail
        pac_low_limb_constraint = cmds.parentConstraint(lower_limb_jnt, detail_lower_limb.ctrl_down.parent_control[1])

        ### TWIST MIDDLE LIMB SETUP
        # create multi matrix
        multi_matrix_ctrl_lower_limb = cmds.shadingNode('multMatrix', asUtility=1,
                                                        n='%s%s%s_mmtx' % (prefix_middle_limb_detail, 'CtrlDown', side))

        cmds.connectAttr(detail_lower_limb.ctrl_down.parent_control[2] + '.worldMatrix[0]',
                         multi_matrix_ctrl_lower_limb + '.matrixIn[0]')
        cmds.connectAttr(detail_lower_limb.ctrl_up.parent_control[0] + '.worldInverseMatrix[0]',
                         multi_matrix_ctrl_lower_limb + '.matrixIn[1]')

        # create decompose matrix
        decompose_matrix_ctrl_lower_limb = cmds.shadingNode('decomposeMatrix', asUtility=1,
                                                            n='%s%s%s_dmtx' % (
                                                                prefix_middle_limb_detail, 'CtrlDown', side))
        cmds.connectAttr(multi_matrix_ctrl_lower_limb + '.matrixSum', decompose_matrix_ctrl_lower_limb + '.inputMatrix')

        # create quat to euler
        quat_ctrl_lower_limb = cmds.shadingNode('quatToEuler', asUtility=1,
                                                n='%s%s%s_qte' % (prefix_middle_limb_detail, 'CtrlDown', side))

        # set rotation order to zxy
        cmds.setAttr(quat_ctrl_lower_limb + '.inputRotateOrder', 2)

        cmds.connectAttr(decompose_matrix_ctrl_lower_limb + '.outputQuatY', quat_ctrl_lower_limb + '.inputQuatY')
        cmds.connectAttr(decompose_matrix_ctrl_lower_limb + '.outputQuatW', quat_ctrl_lower_limb + '.inputQuatW')

        ctrl_lower_bind_mdl_node = []
        for i in range(len(self.set_grp_follicle_twist_middle_limb)):
            value = num * i
            # ctrl down mult double linear
            ctrl_lower_limb_mdl = cmds.shadingNode('multDoubleLinear', asUtility=1,
                                                   n='%s%s%s%s_mdl' % (
                                                       prefix_middle_limb_detail, str(i + 1).zfill(2), 'MultCtrlDown',
                                                       side))
            cmds.setAttr(ctrl_lower_limb_mdl + '.input2', value)
            ctrl_lower_bind_mdl_node.append(ctrl_lower_limb_mdl)

            # connect quatCtrlDown  to respective multiply node
            cmds.connectAttr(quat_ctrl_lower_limb + '.outputRotateY', ctrl_lower_limb_mdl + '.input1')

        for down, twist in zip(ctrl_lower_bind_mdl_node, self.set_grp_follicle_twist_middle_limb):
            cmds.connectAttr(down + '.output', twist + '.rotateY')
        ...
        pac_ctrl_mid_limb_constraint = cmds.parentConstraint(middle_limb_jnt,
        ...
        pt_ctrl_down_constraint = cmds.pointConstraint(self.ctrl_combine_detail,
                                                       detail_upper_limb.ctrl_down.parent_control[2])
        pt_ctrl_up_constraint = cmds.pointConstraint(self.ctrl_combine_detail,
                                                     detail_lower_limb.ctrl_up.parent_control[2])
        rt_utils.constraint_rename(pac_ctrl_mid_limb_constraint)
        rt_utils.constraint_rename(pt_ctrl_down_constraint)
        rt_utils.constraint_rename(pt_ctrl_up_constraint)

        ### CONNECT ROLL
        # disconnect from detail roll to plus minus average
        cmds.disconnectAttr(detail_upper_limb.ctrl_down.parent_control[2] + '.twist',
                            detail_upper_limb.sum_top_pma + '.input1D[0]')
        cmds.disconnectAttr(detail_lower_limb.ctrl_up.parent_control[2] + '.twist',
                            detail_lower_limb.sum_end_pma + '.input1D[0]')

        # connect attribute from respective control module to add double linear control
        cmds.connectAttr(detail_upper_limb.ctrl_down.parent_control[2] + '.twist',
                         build_limb.adl_upper_limb_combine + '.input1')
        cmds.connectAttr(detail_lower_limb.ctrl_up.parent_control[2] + '.twist',
        ...
        cmds.connectAttr(build_limb.adl_upper_limb_combine + '.output', detail_upper_limb.sum_top_pma + '.input1D[0]')
        cmds.connectAttr(build_limb.adl_middle_limb_combine + '.output', detail_lower_limb.sum_end_pma + '.input1D[0]')
        ...
        if detail_limb_deformer:
            ...
            # upper limb
            cmds.connectAttr(build_limb.volume_pos_mdl_upper_limb + '.output',
                             detail_upper_limb.volume_reverse_adl + '.input1')

            # middle limb
            cmds.connectAttr(build_limb.volume_pos_mdl_middle_limb + '.output',
            ...
            for ua, fa in zip(detail_upper_limb.combine_mult_all, detail_lower_limb.combine_mult_all):
                cmds.connectAttr(self.ctrl_combine_detail + '.volumeMultiplier', ua + '.input2')
                cmds.connectAttr(self.ctrl_combine_detail + '.volumeMultiplier', fa + '.input2')

            ### CONNECT SINE SETUP
            cmds.connectAttr(self.ctrl_combine_detail + '.amplitude',
                             detail_upper_limb.combine_sine_amplitude + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.amplitude',
                             detail_lower_limb.combine_sine_amplitude + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.wide', detail_upper_limb.combine_sine_wide + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.wide', detail_lower_limb.combine_sine_wide + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.sineRotate',
                             detail_upper_limb.combine_sine_rotate + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.sineRotate',
                             detail_lower_limb.combine_sine_rotate + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.offset', detail_upper_limb.combine_sine_offset + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.offset', detail_lower_limb.combine_sine_offset + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.twist', detail_upper_limb.combine_sine_twist + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.twist', detail_lower_limb.combine_sine_twist + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.sineLength',
                             detail_upper_limb.combine_sine_length + '.input1')
            cmds.connectAttr(self.ctrl_combine_detail + '.sineLength',
                             detail_lower_limb.combine_sine_length + '.input1')

            # constraint upper limb Dtl Jnt to grp surface
            pac_twist_help_constraint = cmds.parentConstraint(upper_limb_twist_help_driver_jnt,
                                                              detail_upper_limb.grp_surface,
                                                              mo=1)
            rt_utils.constraint_rename(pac_twist_help_constraint)

        # connect the visibility-switch for the controller
        # switch on/off detail module ctrl
        cmds.connectAttr(self.ctrl_combine_detail + '.detailBaseCtrlVis',
        ...
        rt_utils.constraint_rename(
            [pt_upper_limb_ik_constraint[0], aim_lower_limb_ik_constraint[0], elbow_follow_constraint,
    ...
    def single_module_arm(self, base_controller, upper_limb_jnt, middle_limb_jnt, lower_limb_jnt, upper_limb_fk_jnt,
        ...
        self.run_soft_ik_arm = rlu_softIkSetup.run_soft_ik_joint(prefix='arm', side=side,
                                                                 lower_limb_ik_gimbal=self.lower_limb_ik_gimbal,
                                                                 foot_reverse_joint_or_position_soft_jnt=self.pos_soft_jnt,
                                                                 position_lower_limb_jnt=self.pos_lower_limb_jnt,
                                                                 lowerLimbIkControl=self.lower_limb_ik_control)

        cmds.parent(upper_limb_jnt, self.part.joint_grp)
        cmds.parent(self.shoulder_ik_locator, self.shoulder_fk_locator, self.hip_fk_locator, self.hip_ik_locator,
                    self.part.utils_grp)
    ...
    def single_module_leg(self, base_controller, upper_limb_jnt, middle_limb_jnt, lower_limb_jnt, end_limb_jnt,
        ...
        run_soft_ik_leg = rlu_softIkSetup.run_soft_ik_joint(prefix='leg', side=side,
                                                            lower_limb_ik_gimbal=self.lower_limb_ik_gimbal,
                                                            foot_reverse_joint_or_position_soft_jnt=self.pos_soft_jnt,
                                                            position_lower_limb_jnt=self.pos_lower_limb_jnt,
                                                            lowerLimbIkControl=self.lower_limb_ik_control)
    ...
    def combine_volume_detail(self, combine_vol_position_adl, controller, number_detail_ctrl, min_mult=1, mid_mult=1,
                              max_mult=1):
        # min
        cmds.setDrivenKeyframe(combine_vol_position_adl + '.input1',
    ...
    def limb_part_scale(self, prefix, controller, group_scale):
        ...
        for i in group_scale:
            cmds.connectAttr(controller + '.%s%s' % (prefix, 'ScaleX'), i + '.scaleX')
            cmds.connectAttr(controller + '.%s%s' % (prefix, 'ScaleY'), i + '.scaleY')
            cmds.connectAttr(controller + '.%s%s' % (prefix, 'ScaleZ'), i + '.scaleZ')

    def foot_scale(self, controller, ankle_jnt):
        cmds.connectAttr(controller + '.footScaleX', ankle_jnt + '.scaleX')
        cmds.connectAttr(controller + '.footScaleY', ankle_jnt + '.scaleY')
        cmds.connectAttr(controller + '.footScaleZ', ankle_jnt + '.scaleZ')

    def joint_driver(self, arm):
        if arm:
            skel = rlm_tmpSingleModule.all_skeleton(sj_prefix_value='ModArm', ss_prefix_value='ModArmScl',
                                                    sFk_prefix_value='ModArmFk', sIk_prefix_value='ModArmIk',
                                                    sAdd_prefix_value='ModArmShape')
        else:
            skel = rlm_tmpSingleModule.all_skeleton(sj_prefix_value='ModLeg', ss_prefix_value='ModLegScl',
