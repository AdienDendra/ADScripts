...
from rigging.library.base.face import brow as rlbf_brow
...
class Brows:
    def __init__(self,
        ...
        ctrl_driver_grp = cmds.group(em=1, n='browCtrlAll' + '_grp')
        cmds.parent(ctrl_driver_grp, head_up_ctrl_gimbal)
        ...
        left_brow = rlbf_brow.Build(brow_tweak_jnt=brow_tweak_jnt_LFT,
        ...
        right_brow = rlbf_brow.Build(brow_tweak_jnt=brow_tweak_jnt_RGT,
        ...
        center_brow_ctrl = rlu_controller.Control(match_obj_first_position=brow_center_jnt,
        ...
        brow_center_grp = rlu_transform.create_parent_transform(parent_list=['', 'Offset'], object=brow_center_jnt,
                                                                match_position=brow_center_jnt,
                                                                prefix=brow_center_prefix, suffix='_jnt')

        rt_utils.connect_attr_object(center_brow_ctrl.control, brow_center_jnt)
        ...
        cmds.connectAttr(brow_center_grp[1] + '.translateY', center_brow_ctrl.parent_control[1] + '.translateY')
        ...
        cmds.connectAttr(left_brow.brow_mid_jnt_grp[2] + '.translateY',
        ...
        cmds.connectAttr(right_brow.brow_mid_jnt_grp[2] + '.translateY',
                         right_brow.brow_mid_ctrl_grp_offset + '.translateY')

        # PARENT TO THE GROUP
        cmds.parent(center_brow_ctrl.parent_control[0], left_brow.brow_all_ctrl_grp, right_brow.brow_all_ctrl_grp,
                    ctrl_driver_grp)
    ...
    def divide(self, object, side_RGT, side_LFT, side, add_prefix, input_trans2Y=2, target_sum='', target=True):
        ...
        div_node = cmds.createNode('multiplyDivide',
                                   n=rt_utils.prefix_name(new_name) + 'TyDiv' + add_prefix + side + '_mdn')
        cmds.connectAttr(object + '.translateY', div_node + '.input1Y')
        cmds.setAttr(div_node + '.operation', 2)
        cmds.setAttr(div_node + '.input2Y', input_trans2Y)
        if target:
            cmds.connectAttr(div_node + '.outputY', target_sum)
        else:
            return div_node
    ...
    def multiply(self, object_first, object_second, target, side_RGT, side_LFT, side, attr_first, attr_second,
        ...
        div_node = cmds.createNode('multiplyDivide',
                                   n=rt_utils.prefix_name(newName) + 'TyMul' + add_prefix + side + '_mdn')
        cmds.connectAttr(object_first + '.%s' % attr_first, div_node + '.input1Y')
        cmds.connectAttr(object_second + '.%s' % attr_second, div_node + '.input2Y')

        cmds.connectAttr(div_node + '.outputY', target)
        return div_node
    ...
    def sum(self, target_jnt, side_RGT, side_LFT, side=''):
        ...
        sum = cmds.createNode('plusMinusAverage', n=rt_utils.prefix_name(newName) + 'Ty' + side + '_pma')
        cmds.connectAttr(sum + '.output1D', target_jnt)
