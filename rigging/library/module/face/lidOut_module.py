...
class LidOut:
    ...
    # ==================================================================================================================
        ...
        wire_up = wr.Build(curve_template=curve_up_template, position_joint_direction=eyeball_jnt, scale=scale, side_LFT=side_LFT, side_RGT=side_RGT, side=side,
    ...
    # ==================================================================================================================
        ...
        self.corner_multiply_lid_follow(wire=au.prefix_name(wire_up.prefix_name_crv), side=side,
                                        bind_grp_source=wire_up_bind01_grp_offset, bind_grp_destination=wire_up.joint_bind01_grp_corner,
                                        sub_prefix='Bind01',
        ...
        self.corner_multiply_lid_follow(wire=au.prefix_name(wire_low.prefix_name_crv), side=side,
                                        bind_grp_source=wire_low_bind01_grp_offset, bind_grp_destination=wire_low.joint_bind01_grp_corner,
                                        sub_prefix='Bind01',
                                        ctrl_corner_lid=lid_corner_in_ctrl,
                                        attribute=self.lid_in_corner_follow_attr)
        ...
        self.corner_multiply_lid_follow(wire=au.prefix_name(wire_up.prefix_name_crv), side=side,
                                        bind_grp_source=wire_up_bind05_grp_offset, bind_grp_destination=wire_up.joint_bind05_grp_corner,
                                        sub_prefix='Bind05',
                                        ctrl_corner_lid=lid_corner_out_ctrl,
                                        attribute=self.lid_out_corner_follow_attr)

        self.corner_multiply_lid_follow(wire=au.prefix_name(wire_low.prefix_name_crv), side=side,
                                        bind_grp_source=wire_low_bind05_grp_offset, bind_grp_destination=wire_low.joint_bind05_grp_corner,
                                        sub_prefix='Bind05',
                                        ctrl_corner_lid=lid_corner_out_ctrl,
                                        attribute=self.lid_out_corner_follow_attr)
        ...
        self.corner_multiply_lid_follow(wire=au.prefix_name(wire_up.prefix_name_crv), side=side,
                                        bind_grp_source=lid_corner_in_ctrl, bind_grp_destination=lid_corner_ctrl.lid_corner_in_ctrl_grp_offset,
                                        sub_prefix='Ctrl',
                                        ctrl_corner_lid=lid_corner_in_ctrl,
                                        attribute=self.lid_in_corner_follow_attr)

        self.corner_multiply_lid_follow(wire=au.prefix_name(wire_low.prefix_name_crv), side=side,
                                        bind_grp_source=lid_corner_out_ctrl, bind_grp_destination=lid_corner_ctrl.lid_corner_out_ctrl_grp_offset,
                                        sub_prefix='Ctrl',
                                        ctrl_corner_lid=lid_corner_out_ctrl,
                                        attribute=self.lid_out_corner_follow_attr)
    ...
    # ==================================================================================================================
        ...
        mdl_node = mc.createNode('multDoubleLinear', n=au.prefix_name(wire_low.prefix_name_crv) + 'Reverse' + side + '_mdl')
        ...
        multiply = self.multiply_divide(wire_low_name=wire_low.prefix_name_crv + 'Mult', side=side, object_1X=mdl_node + '.output',
                                        object_1Y=corner_lip + '.translateY', object_1Z=corner_lip + '.translateY',
                                        object_2X=corner_lip + '.%s' % corner_lip_attr, object_2Y=corner_lip + '.%s' % corner_lip_attr,
                                        object_2Z=corner_lip + '.%s' % corner_lip_attr, set_attr=False, scale=scale)

        divide = self.multiply_divide(wire_low_name=wire_low.prefix_name_crv + 'Div', side=side, object_1X=multiply + '.outputX',
    ...
    # ==================================================================================================================
        # UP LID
        self.all_ctrl_connecting_lid_out(wire=au.prefix_name(wire_up.prefix_name_crv), side=side, ctrl_bind01=ctrl_bind01_up,
                                         ctrl_bind02=ctrl_bind02_up,
                                         ctrl_bind03=ctrl_bind03_up, ctrl_bind04=ctrl_bind04_up, ctrl_bind05=ctrl_bind05_up,
                                         lid_out_follow=lid_out_follow, drv_bind01_grp_offset=wire_up.joint_bind01_grp_offset,
                                         drv_bind02_grp_offset=wire_up.joint_bind02_grp_offset, drv_bind03_grp_offset=wire_up.joint_bind03_grp_offset,
                                         drv_bind04_grp_offset=wire_up.joint_bind04_grp_offset, drv_bind05_grp_offset=wire_up.joint_bind05_grp_offset,
                                         close_lid_attr=close_lid_attr, upLid=True)
        # LOW LID
        self.all_ctrl_connecting_lid_out(wire=au.prefix_name(wire_low.prefix_name_crv), side=side, ctrl_bind01=ctrl_bind01_low,
                                         ctrl_bind02=ctrl_bind02_low,
                                         ctrl_bind03=ctrl_bind03_low, ctrl_bind04=ctrl_bind04_low, ctrl_bind05=ctrl_bind05_low,
                                         lid_out_follow=lid_out_follow, drv_bind01_grp_offset=wire_low.joint_bind01_grp_offset,
                                         drv_bind02_grp_offset=wire_low.joint_bind02_grp_offset, drv_bind03_grp_offset=wire_low.joint_bind03_grp_offset,
                                         drv_bind04_grp_offset=wire_low.joint_bind04_grp_offset, drv_bind05_grp_offset=wire_low.joint_bind05_grp_offset,
    ...
    #                                                   FUNCTION
    # ==================================================================================================================
    def corner_multiply_lid_follow(self, wire, side, bind_grp_source, bind_grp_destination, sub_prefix, ctrl_corner_lid, attribute):
    ...
    def all_ctrl_connecting_lid_out(self, wire, side, ctrl_bind01, ctrl_bind02, ctrl_bind03, ctrl_bind04, ctrl_bind05,
        ...
        if self.position > 0:
            self.connecting_lid_to_lid_out(wire=wire, side=side, ctrl_bind=ctrl_bind01, number_ctrl='01',
                                           ...
                                           middle_bind_ctrl=False, drv_bind_offset=drv_bind01_grp_offset, close_lid_attr=None,
                                           upLid=upLid, reverse_tx=True)
            self.connecting_lid_to_lid_out(wire=wire, side=side, ctrl_bind=ctrl_bind02, number_ctrl='02',
                                           lid_out_follow_attr=lid_out_follow,
                                           middle_bind_ctrl=False, drv_bind_offset=drv_bind02_grp_offset, close_lid_attr=None,
                                           upLid=upLid, reverse_tx=True)
            self.connecting_lid_to_lid_out(wire=wire, side=side, ctrl_bind=ctrl_bind04, number_ctrl='04',
                                           lid_out_follow_attr=lid_out_follow,
                                           middle_bind_ctrl=False, drv_bind_offset=drv_bind04_grp_offset, close_lid_attr=None,
                                           upLid=upLid, reverse_tx=False)
            self.connecting_lid_to_lid_out(wire=wire, side=side, ctrl_bind=ctrl_bind05, number_ctrl='05',
                                           lid_out_follow_attr=lid_out_follow,
                                           middle_bind_ctrl=False, drv_bind_offset=drv_bind05_grp_offset, close_lid_attr=None,
                                           upLid=upLid, reverse_tx=False)
        else:
            self.connecting_lid_to_lid_out(wire=wire, side=side, ctrl_bind=ctrl_bind01, number_ctrl='01',
                                           lid_out_follow_attr=lid_out_follow,
        ...
        self.connecting_lid_to_lid_out(wire=wire, side=side, ctrl_bind=ctrl_bind03, number_ctrl='03', lid_out_follow_attr=lid_out_follow,
                                       middle_bind_ctrl=True, drv_bind_offset=drv_bind03_grp_offset, close_lid_attr=close_lid_attr, upLid=upLid,
                                       reverse_tx= False)

    def connecting_lid_to_lid_out(self, wire, side, ctrl_bind, number_ctrl, lid_out_follow_attr, middle_bind_ctrl, drv_bind_offset, close_lid_attr, upLid, reverse_tx):
        ...
        if middle_bind_ctrl:
            if upLid:
                ...
                mdl_ty = self.close_lid(wire, side, ctrl_bind, number_ctrl=number_ctrl, sub_prefix='RevTy', attribute='translateY',
                                        value=1)
            ...
            mdn_ty = self.multiply_divide_lid(wire_low_name=wire, side=side, object_1X=mdl_close_lid + '.output', object_2X=ctrl_bind + '.%s' % lid_out_follow_attr,
                                              object_1Y=mdl_ty + '.output', object_2Y=ctrl_bind + '.%s' % lid_out_follow_attr, sub_prefix='Ty', number_ctrl=number_ctrl, two_attr=True)

            cnd_close_lid = self.condition(wire, sub_prefix='CloseLid', obj_condition=ctrl_bind + '.%s' % close_lid_attr,
                                           obj_first=mdn_ty + '.outputX', side=side, number_ctrl=number_ctrl, operation=5)
            ...
            pma = self.plus_minus_avg(wire, sub_prefix='CloseLidTy', side=side, object_first=cnd_close_lid + '.outColorR',
            ...
            mdn_ty_direct = self.multiply_divide_lid(wire_low_name=wire, sub_prefix='DirectTy', side=side, object_1X=mdl_ty + '.output',
                                                     object_2X=ctrl_bind + '.%s' % lid_out_follow_attr, number_ctrl=number_ctrl, two_attr=False)

            cnd_ty_direct = self.condition(wire, sub_prefix='Ty', obj_condition=ctrl_bind + '.translateY',
                                           obj_first=mdn_ty_direct + '.outputX', side=side, number_ctrl=number_ctrl, operation=3)
        ...
        if reverse_tx:
            ...
            mdn_tx = self.multiply_divide_lid(wire_low_name=wire, sub_prefix='Tx', side=side, object_1X=rev_tx + '.output',
                                              object_2X=ctrl_bind + '.%s' % lid_out_follow_attr, number_ctrl=number_ctrl, two_attr=False)
        else:
            mdn_tx = self.multiply_divide_lid(wire_low_name=wire, sub_prefix='Tx', side=side, object_1X=ctrl_bind + '.translateX',
                                              object_2X=ctrl_bind + '.%s' % lid_out_follow_attr, number_ctrl=number_ctrl, two_attr=False)
