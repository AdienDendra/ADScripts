...
from rigging.library.base.face import lid as rlbf_lid, lidCorner as rlbf_lidCorner, irisPupil as rlbf_irisPupil
...
class Lid:
    def __init__(self,
        ...
        lid_grp = cmds.group(em=1, n='lid' + side + '_grp')

        # world up object lid
        world_up_object = cmds.spaceLocator(n='eyeWorldObj' + side + '_loc')[0]
        cmds.delete(cmds.parentConstraint(eyeball_jnt, world_up_object))
        value = cmds.getAttr(world_up_object + '.translateY')
        cmds.setAttr(world_up_object + '.translateY', value + (10 * scale))
        self.world_up_object = world_up_object

        # world up object eye aim
        world_up_aim_object = cmds.duplicate(world_up_object, n='eyeWorldAimObj' + side + '_loc')[0]
        cmds.parent(world_up_aim_object, head_up_ctrl_gimbal)
        cmds.hide(world_up_aim_object)
        self.world_up_aim_object = world_up_aim_object

        self.eye_move_grp = cmds.group(em=1, n='eyeMove' + side + '_grp')
        self.eye_move_grp_offset = cmds.group(em=1, n='eyeMoveOffset' + side + '_grp', p=self.eye_move_grp)
        cmds.delete(cmds.parentConstraint(eyeball_jnt, self.eye_move_grp))

        self.eye_move_all = cmds.group(em=1, n='eyeMoveAll' + side + '_grp')
        cmds.parent(self.eye_move_all, self.eye_move_grp_offset)

        cmds.parent(self.eye_move_grp, lid_grp)

        # LID UP LFT
        self.upLid = rlbf_lid.Build(curve_template=curve_up_lid_template,
        ...
        self.lowLid = rlbf_lid.Build(curve_template=curve_low_lid_template,
                                     world_up_object=world_up_object,
                                     eyeball_jnt=eyeball_jnt,
                                     scale=scale,
                                     offset_jnt02_position=offset_lid02_position,
        ...
        lid_corner_ctrl = rlbf_lidCorner.Build(lid01_up=self.upLid.jnt01,
                                               ...
                                               side=side,
                                               ctrl_shape=rlu_controller.CIRCLEPLUS,
                                               ctrl_color='blue',
                                               suffix_controller=suffix_controller)
        ...
        #                                               IRIS PUPIL
        # ==================================================================================================================
        iris_pupil = rlbf_irisPupil.Build(pupil_jnt=pupil_jnt,
                                          iris_jnt=iris_jnt,
        ...
        cmds.parent(self.upLid.drive_ctrl_grp, self.lowLid.drive_ctrl_grp, lid_corner_ctrl.lid_in_grp,
                    lid_corner_ctrl.lid_out_grp, self.upLid.all_joint_ctrl, self.lowLid.all_joint_ctrl,
                    self.eyeball_ctrl.control)

        cmds.parent(self.upLid.locator_grp, self.upLid.curves_group, self.lowLid.locator_grp, self.lowLid.curves_group,
                    self.eye_move_all)

        cmds.parent(self.upLid.bind_jnt_grp, self.lowLid.bind_jnt_grp, lid_grp)

        cmds.parent(self.eyeball_ctrl.parent_control[0],
                    head_up_ctrl_gimbal)
        cmds.parent(self.upLid.ctrl_0204_grp, self.lowLid.ctrl_0204_grp, self.eyeball_controller)
        cmds.parent(lid_grp, self.upLid.move_grp, self.lowLid.move_grp, face_utils_grp)
    ...
    def part_lid_follow_value_range(self, range_value, side, attr_ctrl, prefix, sub_prefix, attribute, input1='input1X',
        ...
        value_range = cmds.createNode('multiplyDivide', n=prefix_name + prefix + sub_prefix + side + '_mdn')
        cmds.setAttr(value_range + '.operation', 2)
        cmds.setAttr(value_range + '.%s' % input1, range_value)
        cmds.connectAttr(attr_ctrl + '.%s' % attribute, value_range + '.%s' % input2)
    ...
    def part_lid_follow_range_connected(self, side, attr_ctrl, value_range, prefix, sub_prefix, attribute, operation=2,
        ...
        range_side = cmds.createNode('multiplyDivide', n=prefix_name + prefix + sub_prefix + side + '_mdn')
        cmds.setAttr(range_side + '.operation', operation)
        cmds.connectAttr(attr_ctrl + '.%s' % attribute, range_side + '.input1X')
        cmds.connectAttr(value_range + '.outputX', range_side + '.input2X')
    ...
    def corner_lip_to_lid_down(self, range_great, range_less, side, corner_lip, lid_attr, lid_low_ctrl03_grp_offset,
        ...
        pos_mdl = cmds.createNode('multDoubleLinear', n='eyeCornerLipPos' + side + '_mdl')
        cmds.setAttr(pos_mdl + '.input2', positionValue)
        cmds.connectAttr(mult_ctrl_to_range_great_x + '.outputX', pos_mdl + '.input1')

        # CONNECT THE ATTRIBUTE FOR TRANSLATE X
        cmds.connectAttr(pos_mdl + '.output', lid_low_ctrl03_grp_offset + '.translateX')
        cmds.connectAttr(pos_mdl + '.output', lid_low_lip03_bind_corner + '.translateX')

        # CREATE CONDITION FOR GREAT AND LESS
        condition_side = cmds.createNode('condition', n='eyeCornerLipBind' + side + '_cnd')
        cmds.setAttr(condition_side + '.operation', 3)
        cmds.connectAttr(corner_lip + '.translateY', condition_side + '.firstTerm')

        cmds.connectAttr(mult_ctrl_to_range_less_y + '.outputX', condition_side + '.colorIfTrueR')
        cmds.connectAttr(mult_ctrl_to_range_great_y + '.outputX', condition_side + '.colorIfFalseR')

        cmds.connectAttr(condition_side + '.outColorR', lid_low_ctrl03_grp_offset + '.translateY')
        cmds.connectAttr(condition_side + '.outColorR', lid_low_lip03_bind_corner + '.translateY')

    def lid_out_eye_ctrl_connect(self, side, sub_prefix, side_RGT, side_LFT, attribute):
        new_target_up, number_target_up = rlu_transform.reorder_number(prefix=self.eyeball_controller,
                                                                       side_RGT=side_RGT,
                                                                       side_LFT=side_LFT)

        mdn = cmds.createNode('multiplyDivide',
                              n=rt_utils.prefix_name(new_target_up) + sub_prefix + number_target_up + side + '_mdn')
        cmds.connectAttr(self.eyeball_controller + '.%s' % attribute, mdn + '.input1')
        cmds.connectAttr(self.eyeball_controller + '.' + self.lid_out_follow, mdn + '.input2X')
        cmds.connectAttr(self.eyeball_controller + '.' + self.lid_out_follow, mdn + '.input2Y')
        cmds.connectAttr(self.eyeball_controller + '.' + self.lid_out_follow, mdn + '.input2Z')
    ...
    def blink_setup(self, side_RGT, side_LFT, eyeball_jnt, eye_prefix, eye_aim_prefix, curve_up, curve_low, scale,
        ...
        eyeball_grp = rlu_transform.create_parent_transform(parent_list=['Zro', 'Offset'], object=eyeball_jnt,
                                                            match_position=eyeball_jnt,
                                                            prefix='eyeball',
                                                            suffix='_jnt', side=side)

        self.eyeball_ctrl = rlu_controller.Control(match_obj_first_position=eyeball_jnt,
                                                   prefix=eye_prefix,
                                                   shape=rlu_controller.JOINTPLUS, groups_ctrl=['Zro', 'Offset'],
                                                   ctrl_size=scale * 0.5,
        ...
        # ADD ATTRIBUTE
        rt_utils.add_attribute(objects=[self.eyeball_ctrl.control], long_name=['lidDegree'], nice_name=[' '], at="enum",
                               en='Lid Degree', channel_box=True)

        self.eye_aim_follow = rt_utils.add_attribute(objects=[self.eyeball_ctrl.control], long_name=['eyeAimFollow'],
                                                     attributeType="float", min=0.001, dv=1, keyable=True)

        self.lid_out_follow = rt_utils.add_attribute(objects=[self.eyeball_ctrl.control], long_name=['lidOutFollow'],
                                                     attributeType="float", min=0, dv=0.5, keyable=True)

        # EYELID CLOSER
        rt_utils.add_attribute(objects=[self.eyeball_ctrl.control], long_name=['lidCloser'], nice_name=[' '], at="enum",
                               en='Lid Closer', channel_box=True)

        self.lid_position = rt_utils.add_attribute(objects=[self.eyeball_ctrl.control], long_name=['lidPos'],
                                                   attributeType="float", min=0, max=1, dv=0.5, keyable=True)
        self.upLid_closer = rt_utils.add_attribute(objects=[self.eyeball_ctrl.control], long_name=['upLid'],
                                                   attributeType="float", min=-1, max=1, dv=0, keyable=True)
        self.lowLid_closer = rt_utils.add_attribute(objects=[self.eyeball_ctrl.control], long_name=['lowLid'],
                                                    attributeType="float", min=-1, max=1, dv=0, keyable=True)
        # ==============================================================================================================
        #                                                   EYE AIM
        # ==============================================================================================================

        if cmds.xform(eyeball_jnt, q=1, ws=1, t=1)[0] > 0:
            ctrl_color = 'red'
        else:
            ctrl_color = 'yellow'

        self.eye_aim_ctrl = rlu_controller.Control(match_obj_first_position=eye_aim_jnt,
        ...
        get_attribute = cmds.getAttr(self.eye_aim_ctrl.parent_control[0] + '.translateZ')
        cmds.setAttr(self.eye_aim_ctrl.parent_control[0] + '.translateZ',
                     get_attribute + (position_eye_aim_ctrl * scale))

        aim_eye_constraint = cmds.aimConstraint(self.eye_aim_ctrl.control, eyeball_grp[1], mo=1, weight=1,
                                                aimVector=(0, 0, 1), upVector=(0, 1, 0),
                                                worldUpType="object", worldUpObject=world_up_aim_object)

        # CONSTRAINT RENAME
        rt_utils.constraint_rename(aim_eye_constraint)

        # PARENT EYE AIM TO EYE AIM MAIN CTRL
        cmds.parent(self.eye_aim_ctrl.parent_control[0], eye_aim_main_ctrl)

        # MAKE GRP TRANSFORM EYE JNT
        eye_grp = rlu_transform.create_parent_transform(parent_list=[''], object=eye_aim_jnt,
                                                        match_position=eye_aim_jnt,
                                                        prefix='eye',
                                                        suffix='_jnt', side=side)

        # ROTATE THE EYE GROUP
        if self.position >= 0:
            cmds.setAttr(eye_grp[0] + '.rotateY', eye_ctrl_direction)

        else:
            cmds.setAttr(eye_grp[0] + '.rotateY', eye_ctrl_direction * -1)

        # CONNECT THE AIM JNT
        rt_utils.connect_attr_object(self.eyeball_controller, eye_aim_jnt)

        # ==============================================================================================================
        #                                                   BLINK
        # ==============================================================================================================
        # CREATE CURVE MID BLINK
        curve_blink_bind_mid_old = cmds.curve(d=3, ep=[(self.upLid.xform_jnt01), (self.upLid.xform_jnt05)])

        curve_blink_bind_mid_rebuild = cmds.rebuildCurve(curve_blink_bind_mid_old, ch=0, rpo=1, rt=0, end=1, kr=0,
                                                         kcp=0,
                                                         kep=1, kt=0, s=8, d=3, tol=0.01)

        curve_blink_bind_mid = cmds.rename(curve_blink_bind_mid_rebuild, ('lidBlink' + side + '_crv'))

        curve_blink_up = cmds.duplicate(curve_up, n='lidBlinkUp' + side + '_crv')[0]
        curve_blink_low = cmds.duplicate(curve_low, n='lidBlinkLow' + side + '_crv')[0]

        blink_bsn = \
            cmds.blendShape(upLid.deform_curve, lowLid.deform_curve, curve_blink_bind_mid,
                            n=('lidBlink' + side + '_bsn'),
                            weight=[(0, 1), (1, 0)])[0]

        # SKINNING UP AND DOWN BIND JOINT
        # skinning the joint to the bind curve
        skin_cluster = cmds.skinCluster([upLid.jnt05, upLid.jnt04, upLid.jnt01, upLid.jnt02, upLid.jnt03, lowLid.jnt05,
        ...
        cmds.skinPercent(skin_cluster[0], skin_percent_index0, tv=[(upLid.jnt01, 0.5), (lowLid.jnt01, 0.5)])
        cmds.skinPercent(skin_cluster[0], skin_percent_index1,
                         tv=[(upLid.jnt01, 0.45), (upLid.jnt02, 0.05), (lowLid.jnt01, 0.45), (lowLid.jnt02, 0.05)])
        cmds.skinPercent(skin_cluster[0], skin_percent_index2,
        ...
        cmds.skinPercent(skin_cluster[0], skin_percent_index4,
                         tv=[(upLid.jnt02, 0.15), (upLid.jnt03, 0.35), (lowLid.jnt02, 0.15), (lowLid.jnt03, 0.35)])
        cmds.skinPercent(skin_cluster[0], skin_percent_index5, tv=[(upLid.jnt03, 0.5), (lowLid.jnt03, 0.5)])
        cmds.skinPercent(skin_cluster[0], skin_percent_index6,
        ...
        cmds.skinPercent(skin_cluster[0], skin_percent_index8,
                         tv=[(upLid.jnt05, 0.35), (upLid.jnt04, 0.15), (lowLid.jnt05, 0.35), (lowLid.jnt04, 0.15)])
        cmds.skinPercent(skin_cluster[0], skin_percent_index9,
                         tv=[(upLid.jnt05, 0.45), (upLid.jnt04, 0.05), (lowLid.jnt05, 0.45), (lowLid.jnt04, 0.05)])
        cmds.skinPercent(skin_cluster[0], skin_percent_index10, tv=[(upLid.jnt05, 0.5), (lowLid.jnt05, 0.5)])

        cmds.select(cl=1)
        # replace position LFT and RGT
        crv_up_new_name = rlu_transform.reposition_side(object=curve_up, side_RGT=side_RGT, side_LFT=side_LFT)

        # wire deform up on mid curves
        sticky_mid_wire_deformer_up = cmds.wire(curve_blink_up, dds=(0, 100 * scale), wire=curve_blink_bind_mid)
        sticky_mid_wire_deformer_up[0] = cmds.rename(sticky_mid_wire_deformer_up[0],
                                                     (rt_utils.prefix_name(
                                                         crv_up_new_name) + 'Blink' + side + '_wireNode'))
        cmds.setAttr(sticky_mid_wire_deformer_up[0] + '.scale[0]', 0)

        # SET TO LOW CURVE
        cmds.setAttr(blink_bsn + '.%s' % upLid.deform_curve, 0)
        cmds.setAttr(blink_bsn + '.%s' % lowLid.deform_curve, 1)

        cmds.select(cl=1)
        # replace position LFT and RGT
        crv_low_new_name = rlu_transform.reposition_side(object=curve_low, side_RGT=side_RGT, side_LFT=side_LFT)

        sticky_mid_wire_deformer_low = cmds.wire(curve_blink_low, dds=(0, 100 * scale), wire=curve_blink_bind_mid)
        sticky_mid_wire_deformer_low[0] = cmds.rename(sticky_mid_wire_deformer_low[0],
                                                      (rt_utils.prefix_name(
                                                          crv_low_new_name) + 'Blink' + side + '_wireNode'))
        cmds.setAttr(sticky_mid_wire_deformer_low[0] + '.scale[0]', 0)

        # SET KEYFRAME
        cmds.setDrivenKeyframe(blink_bsn + '.%s' % upLid.deform_curve,
        ...
        cmds.setDrivenKeyframe(blink_bsn + '.%s' % lowLid.deform_curve,
                               cd='%s.%s' % (self.eyeball_ctrl.control, self.lid_position),
                               dv=0.5, v=0, itt='linear', ott='linear')
        ...
        up_lid_bsn = cmds.blendShape(curve_blink_up, curve_up, n=('lidBlinkUp' + side + '_bsn'),
                                     weight=[(0, 1)])[0]

        cmds.connectAttr(self.eyeball_controller + '.%s' % self.upLid_closer, up_lid_bsn + '.%s' % curve_blink_up)

        low_lid_bsn = cmds.blendShape(curve_blink_low, curve_low, n=('lidBlinkLow' + side + '_bsn'),
                                      weight=[(0, 1)])[0]

        cmds.connectAttr(self.eyeball_controller + '.%s' % self.lowLid_closer, low_lid_bsn + '.%s' % curve_blink_low)

        # parent eyeblink crve to face curve grp
        cmds.parent(curve_blink_bind_mid, cmds.listConnections(sticky_mid_wire_deformer_low[0] + '.baseWire[0]')[0],
                    cmds.listConnections(sticky_mid_wire_deformer_up[0] + '.baseWire[0]')[0], base_module_nonTransform)
    ...
    def part_lid_bind_follow(self, range_valueA, range_valueB, range_valueC, range_valueD, eye_aim_follow,
        ...
        condition_aim_side_greater = cmds.createNode('condition', n='eyeAimBind' + prefix + side + '_cnd')
        cmds.setAttr(condition_aim_side_greater + '.colorIfTrueG', 0)
        cmds.connectAttr(eye_aim_ctrl + '.translateY', condition_aim_side_greater + '.firstTerm')
        ...
        range_aim_bind_b = self.part_lid_follow_range_connected(side, eye_aim_ctrl, value_range=value_range_b,
                                                                ...
                                                                attribute='translateY')
        cmds.connectAttr(range_aim_bind_b + '.outputX', condition_aim_side_greater + '.colorIfTrueR')
        ...
        if up_lid:
            ...
            cmds.setAttr(condition_aim_side_greater + '.operation', 5)
            range_aim_bindD = self.part_lid_follow_range_connected(side, eye_aim_ctrl, value_range=value_rangeD,
                                                                   ...
                                                                   attribute='translateY')
            cmds.connectAttr(range_aim_bindD + '.outputX', condition_aim_side_greater + '.colorIfFalseR')
        ...
        # CREATE DIVIDE RANGE AIM SIDE A
            ...
            cmds.setAttr(condition_aim_side_greater + '.operation', 3)
            range_aim_bindA = self.part_lid_follow_range_connected(side, eye_aim_ctrl, value_range=value_rangeA,
                                                                   ...
                                                                   attribute='translateY')
            cmds.connectAttr(range_aim_bindA + '.outputX', condition_aim_side_greater + '.colorIfFalseR')

        cmds.connectAttr(condition_aim_side_greater + '.outColorR', lid_bind03_grp + '.translateY')
        ...
        range_aim_bindC = self.part_lid_follow_range_connected(side, eye_aim_ctrl, value_range=value_range_c,
                                                               ...
                                                               attribute='translateX')
        cmds.connectAttr(range_aim_bindC + '.outputX', lid_bind03_grp + '.translateX')
    ...
    def lid_follow_ctrl(self, eye_aim_follow, eye_ctrl, eye_aim_side_ctrl, side,
        ...
        sum_aim_side_and_aimC = cmds.createNode('plusMinusAverage', n='eyeAimCtrlRangeC' + side + '_pma')
        cmds.connectAttr(range_aimC + '.outputX', sum_aim_side_and_aimC + '.input1D[0]')
        cmds.connectAttr(range_aim_sideC + '.outputX', sum_aim_side_and_aimC + '.input1D[1]')
        ...
        sum_aim_side_and_aimA = cmds.createNode('plusMinusAverage', n='eyeAimCtrlRangeA' + side + '_pma')
        cmds.connectAttr(range_aimA + '.outputX', sum_aim_side_and_aimA + '.input1D[0]')
        cmds.connectAttr(range_aim_sideA + '.outputX', sum_aim_side_and_aimA + '.input1D[1]')
        ...
        range_aimB_rev = cmds.createNode('multDoubleLinear', n='eyeAimCtrlRangeBRev' + side + '_mdl')
        cmds.setAttr(range_aimB_rev + '.input1', -1)
        cmds.connectAttr(range_aimB + '.outputX', range_aimB_rev + '.input2')

        sum_aim_side_and_aimB = cmds.createNode('plusMinusAverage', n='eyeAimCtrlRangeB' + side + '_pma')
        cmds.setAttr(sum_aim_side_and_aimB + '.operation', 2)
        cmds.connectAttr(range_aimB_rev + '.output', sum_aim_side_and_aimB + '.input1D[0]')
        cmds.connectAttr(range_aim_sideB + '.outputX', sum_aim_side_and_aimB + '.input1D[1]')

        cmds.connectAttr(sum_aim_side_and_aimC + '.output1D', lid_up_ctrl03_grp_offset + '.translateX')
        cmds.connectAttr(sum_aim_side_and_aimA + '.output1D', lid_up_ctrl03_grp_offset + '.translateY')
        cmds.connectAttr(sum_aim_side_and_aimC + '.output1D', lid_low_ctrl03_grp_offset + '.translateX')
        cmds.connectAttr(sum_aim_side_and_aimB + '.output1D', lid_low_ctrl03_grp_offset + '.translateY')
