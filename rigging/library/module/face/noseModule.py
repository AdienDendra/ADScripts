...
from rigging.library.base.face import wire as rlbf_wire, nose as rlbf_nose
...
class Nose:
    def __init__(self,
        ...
        #                                               NOSE CONTROLLER
        # ==================================================================================================================
        columella_ctrl = rlbf_nose.Build(columella_jnt, columella_prefix, suffix_controller, scale)

        wire = rlbf_wire.Build(curve_template=curve_template_nose, position_joint_direction=nose_jnt, scale=scale,
        ...
        # ADD ATTRIBUTE LFT
        rt_utils.add_attribute(objects=[controller_bind01_LFT], long_name=['cheekUpSetup'], nice_name=[' '], at="enum",
                               en='Cheek Up Setup', channel_box=True)
        self.pull_forward_LFT = rt_utils.add_attribute(objects=[controller_bind01_LFT], long_name=['pullForward'],
                                                       attributeType="float", min=0.001, max=10, dv=1, keyable=True)

        self.push_upward_LFT = rt_utils.add_attribute(objects=[controller_bind01_LFT], long_name=['pushUpward'],
                                                      attributeType="float", min=0.001, max=10, dv=1, keyable=True)

        # ADD ATTRIBUTE RGT
        rt_utils.add_attribute(objects=[controller_bind01_RGT], long_name=['cheekUpSetup'], nice_name=[' '], at="enum",
                               en='Cheek Up Setup', channel_box=True)
        self.pull_forward_RGT = rt_utils.add_attribute(objects=[controller_bind01_RGT], long_name=['pullForward'],
                                                       attributeType="float", min=0.001, max=10, dv=1, keyable=True)

        self.push_upward_RGT = rt_utils.add_attribute(objects=[controller_bind01_RGT], long_name=['pushUpward'],
                                                      attributeType="float", min=0.001, max=10, dv=1, keyable=True)

        # ADD NOSE CONTROLLER
        nose_controller = rlu_controller.Control(match_obj_first_position=nose_jnt,
                                                 prefix='nose',
        ...
        # ADD ATTRIBUTE NOSE
        rt_utils.add_attribute(objects=[nose_controller.control], long_name=['weightSkinInfluence'], nice_name=[' '],
                               at="enum",
                               en='Weight Influence', channel_box=True)

        self.mouth_weight_ss = rt_utils.add_attribute(objects=[nose_controller.control], long_name=['squashStretch'],
                                                      attributeType="float", min=0, dv=1, keyable=True)

        self.mouth_weight_up_down = rt_utils.add_attribute(objects=[nose_controller.control], long_name=['upDown'],
                                                           attributeType="float", min=0, dv=1, keyable=True)
        # OFFSET POSITION PARENT GRP NOSE
        tZ_value = cmds.getAttr(self.nose_ctrl_grp_zro + '.translateZ')
        cmds.setAttr(self.nose_ctrl_grp_zro + '.translateZ', tZ_value + (position_mouth_ctrl * 0.5 * scale))

        # CREATE PMA FOR EXPRESSION
        self.nose_ctrl_translate_pma = cmds.createNode('plusMinusAverage', n='noseWeightToMouthTrans_pma')
        self.nose_ctrl_rotate_pma = cmds.createNode('plusMinusAverage', n='noseWeightToMouthRot_pma')
        self.nose_ctrl_scale_pma = cmds.createNode('plusMinusAverage', n='noseWeightToMouthScl_pma')

        # CONNECT ATTRIBUTE NOSE CTRL TO GRP OFFSET JOINT DRIVEN AND CONTROLLER
        cmds.connectAttr(self.nose_ctrl + '.translate', self.nose_ctrl_translate_pma + '.input3D[0]')
        cmds.connectAttr(self.nose_ctrl_grp_offset + '.translate', self.nose_ctrl_translate_pma + '.input3D[1]')

        cmds.connectAttr(self.nose_ctrl + '.rotate', self.nose_ctrl_rotate_pma + '.input3D[0]')
        cmds.connectAttr(self.nose_ctrl + '.scale', self.nose_ctrl_scale_pma + '.input3D[0]')

        cmds.connectAttr(self.nose_ctrl_rotate_pma + '.output3D', wire.wire_driven_jnt_grp_offset + '.rotate')
        # au.connectAttrRot(self.noseCtrl, noseGrpDrivenOffsetJnt)
        cmds.connectAttr(self.nose_ctrl_scale_pma + '.output3D', wire.wire_driven_jnt_grp_offset + '.scale')

        cmds.connectAttr(self.nose_ctrl_translate_pma + '.output3D', wire.wire_driven_ctrl_grp_offset + '.translate')
        cmds.connectAttr(self.nose_ctrl_rotate_pma + '.output3D', wire.wire_driven_ctrl_grp_offset + '.rotate')
        # au.connectAttrRot(self.noseCtrl, noseGrpDrivenOffsetCtrl)
        cmds.connectAttr(self.nose_ctrl_scale_pma + '.output3D', wire.wire_driven_ctrl_grp_offset + '.scale')

        cmds.connectAttr(self.nose_ctrl_translate_pma + '.output3D', columella_ctrl.columella_jnt_grp[1] + '.translate')
        cmds.connectAttr(self.nose_ctrl_rotate_pma + '.output3D', columella_ctrl.columella_jnt_grp[1] + '.rotate')
        # au.connectAttrRot(self.noseCtrl, columellaCtrl.grpColumellaJnt[1])
        cmds.connectAttr(self.nose_ctrl_scale_pma + '.output3D', columella_ctrl.columella_jnt_grp[1] + '.scale')

        # CONNECT ALL PART NOSE JOINT
        for i in wire.all_joint:
            cmds.connectAttr(self.nose_ctrl_translate_pma + '.output3D', i + '.translate')
            cmds.connectAttr(self.nose_ctrl_rotate_pma + '.output3D', i + '.rotate')
            # au.connectAttrRot(self.noseCtrl, i)
            cmds.connectAttr(self.nose_ctrl_scale_pma + '.output3D', i + '.scale')
            # au.connectAttrObject(self.noseCtrl, i)

        # PARENT INTO THE GROUP
        cmds.parent(columella_ctrl.columella_ctrl_grp_zro, wire.wire_driven_ctrl_grp_offset)
        cmds.parent(wire.joint_grp, head_jnt)
        cmds.parent(wire.ctrl_driver_grp, self.nose_ctrl_grp_zro, head_ctrl_gimbal)

        # ADD NOSE UP CONTROLLER
        nose_up_controller = rlu_controller.Control(match_obj_first_position=nose_up_jnt,
                                                    prefix='noseUp',
                                                    shape=rlu_controller.JOINT, groups_ctrl=['Zro'],
                                                    ctrl_size=scale * 0.05,
                                                    ctrl_color='turquoiseBlue', lock_channels=['v'])

        self.nose_up_controller_grp = nose_up_controller.parent_control[0]

        grp_nose_up_jnt = rlu_transform.create_parent_transform(parent_list=['Zro'], object=nose_up_jnt,
                                                                match_position=nose_up_jnt,
                                                                prefix='noseUp', suffix='_jnt')

        rt_utils.connect_attr_object(nose_up_controller.control, nose_up_jnt)
        cmds.parent(nose_up_controller.parent_control[0], head_up_ctrl_gimbal)
        ...
        get_attr_grp_zro_bind_nostril_LFT = cmds.getAttr(joint_bind05_grp + '.translate')[0]
        get_attr_grp_zro_bind_nostril_RGT = cmds.getAttr(joint_bind01_grp + '.translate')[0]
        ...
        cmds.connectAttr(left_side[0] + '.output1D', joint_bind05_grp + '.translateX')
        cmds.connectAttr(left_side[1] + '.output1D', joint_bind05_grp + '.translateY')
        cmds.connectAttr(left_side[2] + '.output1D', joint_bind05_grp + '.translateZ')

        cmds.connectAttr(right_side[0] + '.output1D', joint_bind01_grp + '.translateX')
        cmds.connectAttr(right_side[1] + '.output1D', joint_bind01_grp + '.translateY')
        cmds.connectAttr(right_side[2] + '.output1D', joint_bind01_grp + '.translateZ')
        ...
        cmds.connectAttr(sum_squash_stretch + '.outColorR', self.nose_ctrl_scale_pma + '.input3D[1].input3Dx')
        cmds.connectAttr(sum_squash_stretch + '.outColorG', self.nose_ctrl_scale_pma + '.input3D[1].input3Dy')
        ...
        cmds.connectAttr(multiply_up_down_nose + '.output', self.nose_ctrl_grp_offset + '.translateY')
    ...
    def squash_stretch_great_less(self, jaw_ctrl, input_1X_great, input_2X_great, input_1Y_great, input_2Y_great,
        ...
        condition = cmds.createNode('condition', n='nostrilWeightSquashStretch_cnd')
        cmds.setAttr(condition + '.operation', 3)
        cmds.connectAttr(jaw_ctrl + '.rotateX', condition + '.firstTerm')
        cmds.connectAttr(less + '.outputX', condition + '.colorIfFalseR')
        cmds.connectAttr(less + '.outputY', condition + '.colorIfFalseG')
        cmds.connectAttr(great + '.outputX', condition + '.colorIfTrueR')
        cmds.connectAttr(great + '.outputY', condition + '.colorIfTrueG')
    ...
    def connect_attr_XY_value_weight(self, name, input_1X, input_2X, input_1Y, input_2Y, operation, prefix):
        ...
        cmds.connectAttr(input_1X, ctrl_drv_mdn + '.input1X')
        cmds.connectAttr(input_2X, ctrl_drv_mdn + '.input2X')
        cmds.connectAttr(input_1Y, ctrl_drv_mdn + '.input1Y')
        cmds.connectAttr(input_2Y, ctrl_drv_mdn + '.input2Y')
    ...
    def set_attribute_value_origin(self, value_translate, nostril_attribute_mdn, axis, side):
        ctrl_drv_pma = cmds.createNode('plusMinusAverage', n='nostriWeightSetOrigin' + axis + side + '_pma')
        cmds.setAttr(ctrl_drv_pma + '.operation', 1)
        cmds.setAttr(ctrl_drv_pma + '.input1D[0]', value_translate)
        cmds.connectAttr(nostril_attribute_mdn + '.outputX', ctrl_drv_pma + '.input1D[1]')
    ...
    def add_all_value_controller_squash_stretch(self, name, prefix, multiply_divide_output_second,
        ...
        ctrl_drv_pma = cmds.createNode('plusMinusAverage', n=name + prefix + side + '_pma')
        cmds.setAttr(ctrl_drv_pma + '.operation', operation)
        cmds.connectAttr(multiply_divide_output_first, ctrl_drv_pma + '.input1D[0]')
        cmds.connectAttr(multiply_divide_output_second, ctrl_drv_pma + '.input1D[1]')
    ...
    def add_all_value_controller(self, multiply_divide_output_mouth, multiply_divide_output_corner_lip,
        ...
        ctrl_drv_pma = cmds.createNode('plusMinusAverage', n='nostriWeightCombine' + axis + 'Ctrl' + side + '_pma')
        cmds.setAttr(ctrl_drv_pma + '.operation', 1)
        cmds.connectAttr(multiply_divide_output_mouth + '.outputX', ctrl_drv_pma + '.input1D[0]')
        cmds.connectAttr(multiply_divide_output_corner_lip + '.outputX', ctrl_drv_pma + '.input1D[1]')
        cmds.connectAttr(multiply_divide_output_up_lip + '.outputX', ctrl_drv_pma + '.input1D[2]')

        return ctrl_drv_pma

    def set_value_weight(self, name, input_2X_set, input_1X, operation, prefix, name_expression, side):
        ctrl_drv_mdn = cmds.createNode('multiplyDivide', n=name + prefix + name_expression + 'Ctrl' + side + '_mdn')
        cmds.setAttr(ctrl_drv_mdn + '.operation', operation)
        cmds.connectAttr(input_1X, ctrl_drv_mdn + '.input1X')
        cmds.setAttr(ctrl_drv_mdn + '.input2X', input_2X_set)

        return ctrl_drv_mdn

    def connect_value_weight(self, name, input_2X, input_1X, operation, prefix, name_expression, axis, side):
        ctrl_drv_mdn = cmds.createNode('multiplyDivide',
                                       n=name + prefix + name_expression + axis + 'Ctrl' + side + '_mdn')
        cmds.setAttr(ctrl_drv_mdn + '.operation', operation)
        cmds.connectAttr(input_1X, ctrl_drv_mdn + '.input1X')
        cmds.connectAttr(input_2X, ctrl_drv_mdn + '.input2X')
    ...
    def mdl_set_attr_up_down(self, name, prefix, name_expression, side, input1, input2):
        corner_lip_range_mdl = cmds.createNode('multDoubleLinear',
                                               n=name + prefix + name_expression + 'Ctrl' + side + '_mdl')
        cmds.connectAttr(input1, corner_lip_range_mdl + '.input1')
        cmds.connectAttr(input2, corner_lip_range_mdl + '.input2')
    ...
    def mdl_set_attr(self, name, prefix, name_expression, side, input1, input2_set):
        corner_lip_range_mdl = cmds.createNode('multDoubleLinear',
                                               n=name + prefix + name_expression + 'Ctrl' + side + '_mdl')
        cmds.connectAttr(input1, corner_lip_range_mdl + '.input1')
        cmds.setAttr(corner_lip_range_mdl + '.input2', input2_set)
