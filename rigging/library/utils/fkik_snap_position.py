...
def fk_to_ik(axis_aim='translateY'):
    ...
    if getattr_ctrl == 1:
        ...
        middle_ref_jnt = mc.listConnections(fkik_ctrl_select[0] + '.middle_ref_jnt')[0]
        lower_ref_jnt = mc.listConnections(fkik_ctrl_select[0] + '.lower_ref_jnt')[0]
        fk_ik_arm_ctrl = mc.listConnections(fkik_ctrl_select[0] + '.fk_ik_arm_ctrl', s=1)
        fk_ik_leg_ctrl = mc.listConnections(fkik_ctrl_select[0] + '.fk_ik_leg_ctrl', s=1)
...
def fk_to_ik_setup(upper_limb_snap_jnt, middle_limb_snap_jnt, lower_limb_snap_jnt,
    ...
    if leg :
        ...
        xform_end_limb_rot = mc.getAttr(end_limb_snap_jnt + '.rotateX')
    ...
    value_axis_middle_jnt = mc.getAttr('%s.%s' % (value_axis_aim_middle_jnt, axis_towards))
    value_axis_lower_jnt = mc.getAttr('%s.%s' % (value_axis_aim_lower_jnt, axis_towards))
    ...
    current_value_axis_towards_middle_jnt = mc.getAttr('%s.%s' % (middle_limb_snap_jnt, axis_towards))
    current_value_axis_towards_lower_jnt = mc.getAttr('%s.%s' % (lower_limb_snap_jnt, axis_towards))
