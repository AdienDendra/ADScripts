...
class CreateRibbon:
    def __init__(self,
        ...
        if parallelAxis in direction.keys():
            mc.setAttr(tmpPlane + direction[parallelAxis][0], direction[parallelAxis][1])
        else:
            mc.error('The string %s in parallelFaceAxis argumment is not found. Fill with x or y or z ' % parallelAxis)
        ...
        mc.setAttr((geoPlaneVolumeBsn[0] + direction[parallelAxis][2]), -0.5)
        ...
        self.ctrlUpPos   = mc.setAttr((ctrlUp.parent_control[0] + '.translate'), ctrlPos[parallelAxis][0], ctrlPos[parallelAxis][1], ctrlPos[parallelAxis][2])
        self.ctrlDownPos = mc.setAttr((ctrlDown.parent_control[0] + '.translate'), ctrlPos[parallelAxis][3], ctrlPos[parallelAxis][4], ctrlPos[parallelAxis][5])
        ...
        mc.setAttr((self.jointUp[0] + '.translate'), ctrlPos[parallelAxis][0], ctrlPos[parallelAxis][1], ctrlPos[parallelAxis][2])
        mc.setAttr((self.jointMid[0] + '.translate'),self.pos[0], self.pos[1], self.pos[2])
        mc.setAttr((self.jointDown[0] + '.translate'), ctrlPos[parallelAxis][3], ctrlPos[parallelAxis][4], ctrlPos[parallelAxis][5])

        # set the position for joints orientation ribbon
        mc.setAttr((self.jointOrientUp[0] + '.translate'), ctrlPos[parallelAxis][0], ctrlPos[parallelAxis][1], ctrlPos[parallelAxis][2])
        mc.setAttr((self.jointOrientMid[0] + '.translate'),self.pos[0], self.pos[1], self.pos[2])
        mc.setAttr((self.jointOrientDown[0] + '.translate'), ctrlPos[parallelAxis][3], ctrlPos[parallelAxis][4], ctrlPos[parallelAxis][5])
