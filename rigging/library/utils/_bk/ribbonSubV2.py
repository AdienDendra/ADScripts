...
class CreateRibbon:
    def __init__(self,
        ...
        tipPoint ={'x+': [(width / 2.0 * -1), (width / 2.0)],
                   'x-': [(width / 2.0), (width / 2.0 * -1)],
                   'y+': [(width / 2.0 * -1), (width / 2.0)],
                   'y-': [(width / 2.0), (width / 2.0 * -1)],
                   'z+': [(width / 2.0 * -1), (width / 2.0)],
                   'z-': [(width / 2.0), (width / 2.0 * -1)]
                   }
        ...
        direction = {'x' : ['.rotateY', 90, '.translateX'],
                     'y' : ['.rotateZ', 90, '.translateZ'],
                     'z' : ['.rotateX', 0, '.translateZ']}

        ctrlPos = {'x' : [self.pos[0], self.pos[1], self.upPoint, self.pos[0], self.pos[1], self.downPoint],
                   'y' : [self.pos[0], self.upPoint, self.pos[2], self.pos[0], self.downPoint, self.pos[2]],
                   'z' : [self.upPoint, self.pos[1], self.pos[2], self.downPoint, self.pos[1], self.pos[2]]}

        rotDefomer = {'x' : (-90, 0, 90),
                      'y' : (0, 0, 180),
                      'z' : (0, 0, 90)}

        # rotSkin = {'x' =
        volInputMdn = {'x': ['.translateZ','.input1X', '.outputX'],
                       'y': ['.translateY','.input1Z', '.outputZ'],
                       'z': ['.translateX','.input1Z', '.outputZ']}


        if front in direction.keys():
            mc.setAttr(tmpPlane + direction[front][0], direction[front][1])
        ...
        geoPlaneVolumeBsn = mc.duplicate(tmpPlane, name=(prefix + 'VolumeBsn_geo'))

        # Offset the volume-plane
        if front in direction.keys():
            mc.setAttr((geoPlaneVolume[0] + direction[front][2]), -0.5)
            mc.setAttr((geoPlaneVolumeBsn[0] + direction[front][2]), -0.5)
       ...
       # Create the controllers
        ...
        mc.delete(mc.orientConstraint(tmpPlane, ctrlUp.parent_control[0]))
        mc.delete(mc.orientConstraint(tmpPlane, ctrlMid.parent_control[0]))
        mc.delete(mc.orientConstraint(tmpPlane, ctrlDown.parent_control[0]))
        ...
        # position
        if front in ctrlPos.keys():
            self.ctrlUpPos   = mc.setAttr((ctrlUp.parent_control[0] + '.translate'), ctrlPos[front][0], ctrlPos[front][1], ctrlPos[front][2])
            self.ctrlDownPos = mc.setAttr((ctrlDown.parent_control[0] + '.translate'), ctrlPos[front][3], ctrlPos[front][4], ctrlPos[front][5])
        ...
        mc.delete(mc.orientConstraint(tmpPlane, deformCrv))
        ...
        mc.delete(mc.orientConstraint(tmpPlane, jointCreate))
        mc.makeIdentity(jointCreate, apply=True, r=True, n=1)
        ...
        # # set the position for joints orientation ribbon
        ...
        # set the position for joints wire curve
        if front in ctrlPos:
            mc.setAttr((self.jointUp[0] + '.translate'),ctrlPos[front][0], ctrlPos[front][1], ctrlPos[front][2])
            mc.setAttr((self.jointMid[0] + '.translate'),self.pos[0], self.pos[1], self.pos[2])
            mc.setAttr((self.jointDown[0] + '.translate'), ctrlPos[front][3], ctrlPos[front][4], ctrlPos[front][5])

            # set the position for joints orientation ribbon
            mc.setAttr((self.jointOrientUp[0] + '.translate'),ctrlPos[front][0], ctrlPos[front][1], ctrlPos[front][2])
            mc.setAttr((self.jointOrientMid[0] + '.translate'),self.pos[0], self.pos[1], self.pos[2])
            mc.setAttr((self.jointOrientDown[0] + '.translate'), ctrlPos[front][3], ctrlPos[front][4], ctrlPos[front][5])
        ...
        self.jointUpOrientParent   = tr.create_parent_transform(['Zro', 'Offset'], self.jointOrientUp[0], self.jointOrientUp[0])
        self.jointMidOrientParent  = tr.create_parent_transform(['Zro', 'Offset'], self.jointOrientMid[0], self.jointOrientMid[0])
        self.jointDownOrientParent = tr.create_parent_transform(['Zro', 'Offset'], self.jointOrientDown[0], self.jointOrientDown[0])


        # decompose vector orient of joint orientation
        # mc.delete(mc.orientConstraint(module, self.jointUpOrientParent[0], mo=0))
        # mc.delete(mc.orientConstraint(tip, self.jointDownOrientParent[0], mo=0))
        # mc.delete(mc.orientConstraint(module, tip, self.jointMidOrientParent[0], mo=0))

        cr.decompose_matrix_axis(xAxis, yAxis, zAxis, self.jointUpOrientParent[0])
        cr.decompose_matrix_axis(xAxis, yAxis, zAxis, self.jointMidOrientParent[0])
        cr.decompose_matrix_axis(xAxis, yAxis, zAxis, self.jointDownOrientParent[0])

        #
        # geoPlaneOrientShape = mc.listRelatives(geoPlaneOrient[0], type="shape")
        # follicleOrientBase = self.itemFollicle(self.jointOrientUp, geoPlaneOrientShape, 'Uptemp')
        # follicleOrientTip  = self.itemFollicle(self.jointOrientDown, geoPlaneOrientShape, 'Downtemp')
        #
        # posBasePCons = mc.parentConstraint(follicleOrientBase['follicle'], self.jointOrientUp[0])
        # posTipPCons = mc.parentConstraint(follicleOrientTip['follicle'], self.jointOrientDown[0])
        #
        # mc.delete(mc.orientConstraint(module,tip, geoPlaneOrient))
        # mc.delete(posBasePCons, posTipPCons, follicleOrientBase['follicle'], follicleOrientTip['follicle'])
        #
        # mc.delete(mc.orientConstraint(module, self.jointOrientUp[0]))
        # mc.delete(mc.orientConstraint(tip, self.jointOrientDown[0]))
        # mc.delete(mc.orientConstraint(module, self.jointOrientMid[0]))
        #
        # mc.makeIdentity(self.jointOrientUp[0], apply=True, r=True, n=1)
        # mc.makeIdentity(self.jointOrientDown[0], apply = True, r = True, n = 1)
        # mc.makeIdentity(self.jointOrientMid[0], apply=True, r=True, n=1)
        #
        # # Create Group for joints orientation ribbon
        # self.jointUpOrientParent   = tr.createParentTransform(['Zro','Offset'], self.jointOrientUp[0], self.jointOrientUp[0])
        # self.jointMidOrientParent  = tr.createParentTransform(['Zro','Offset'], self.jointOrientMid[0], self.jointOrientMid[0])
        # self.jointDownOrientParent = tr.createParentTransform(['Zro','Offset'], self.jointOrientDown[0], self.jointOrientDown[0])

        #
        mc.delete(mc.orientConstraint(base, self.jointUpParent[0], mo=0))
        mc.delete(mc.orientConstraint(tip, self.jointMidParent[0], mo=0))
        mc.delete(mc.orientConstraint(base, tip, self.jointDownParent[0], mo=0))
        ...
        mc.skinCluster([self.jointOrientUp[0], self.jointOrientMid[0], self.jointOrientDown[0]], geoPlaneOrient,
                       n='orientSkinCluster', tsb=True, bm=0, sm=0, nw=1, mi=1)
        ...
        mc.blendShape(geoPlaneVolume[0], geoPlaneVolumeBsn[0], name=(prefix + 'Vol_bsn'), weight=[(0, 1)])
        ...
        mc.makeIdentity(geoPlane[0], geoPlaneVolumeBsn[0], apply=True, r=True, n=1)
        ...
        if front in volInputMdn.keys():
            mc.connectAttr((ctrlMid.control + '.volumePosition'), (self.squashDef[1] + volInputMdn[front][0]))
        ...
        # Object shape geo plane
        geoPlaneShape    = mc.listRelatives(geoPlane[0], type="shape")
        geoPlaneShapeVol = mc.listRelatives(geoPlaneVolumeBsn[0], type="shape")

        # Create follicles: The main-surface and the volume-surface
        follicleS = self.itemFollicle(positionUVFol, geoPlaneShape, 'fol')
        follicleV = self.itemFollicle(positionUVFol, geoPlaneShapeVol, 'Volumefol')

        # Set the follicle V parameter to -1
        for obj in follicleV['follicle']:
            mc.setAttr(obj + '.parameterV', -1)
        ...
        for folS, folV in zip (follicleS['follicle'], follicleV['follicle']):
            ...
            follicleCtrl = ct.Control(prefix =au.prefix_name(folS), groups_ctrl=['Zro', 'Offset'], ctrl_color='lightPink', shape=ctrlDetails)
            ...
            if front in volInputMdn.keys():
                ...
                mc.connectAttr((ctrlMid.control + '.volumeMultiplier'), volInputMdn[front][1])
                ...
                mc.connectAttr((multMpd + volInputMdn[front][2]), (sumPma + '.input1D[0]'))
        ...
        # # Constraining the control ribbon to module and tip
        # mc.parentConstraint(module, ctrlUp.parentControl[-1])
        # mc.parentConstraint(tip, ctrlDown.parentControl[-1])
        #

        # # Match the position and orientation joint wire curve to module and tip joint
        # mc.delete(mc.orientConstraint(module, self.jointUpParent[0]))
        # mc.delete(mc.orientConstraint(tip, self.jointDownParent[0]))
        # mc.delete(mc.orientConstraint(module, self.jointMidParent[0]))
        # mc.delete(mc.orientConstraint(ctrlMid.control, self.jointMidParent[0]))
        #
        # # Match position controller with the module and tip joint
        # mc.delete(mc.parentConstraint(module, ctrlUp.parentControl[0]))
        # mc.delete(mc.parentConstraint(tip, ctrlDown.parentControl[0]))
        #
        # # Match the position of transform group
        # mc.delete(mc.parentConstraint(module, tip, self.grpTransform))

        # # Match the orientation of mid transform group
        # mc.delete(mc.orientConstraint(ctrlMid.control, self.jointMidParent[0]))

        #
        # Parent joint module and tip to all ribbon grp


        # Cleanup: Hierarchy
        mc.parent(base, tip, self.grpAllRibbon)
        mc.parent(self.twistDef[1], self.sineDef[1], self.squashDef[1], self.grpDeformers)
        mc.parent(geoPlaneOrient[0], geoPlaneWire[0], geoPlaneTwist[0], geoPlaneSine[0], geoPlaneVolume[0],
                  geoPlaneVolumeBsn[0], self.grpSurfaces)
        ...
        # mc.parent(self.jointUpParent[0], self.jointMidParent[0], self.jointDownParent[0], self.grpJntCluster[0])
        ...
        # Cleanup: Visibility
        mc.hide(self.grpFollVolume, self.grpSurfaces, self.grpDeformers, self.grpMisc, self.grpJntCluster)
