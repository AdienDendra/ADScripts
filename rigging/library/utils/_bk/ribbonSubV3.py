...
class CreateRibbon:
    def __init__(self,
        ...
        :param parallelFaceAxis     : str, two point parallel position direction whether on x or y or z axis
        :param tipPos    : str, position point (joint) of the tip whether on +x +y +z or -x -y -z axis
        ...
        :param aimAxis   : str, aim axis of the for rotation joint. Can fill with +x +y +z or -x -y -z
        ...
        width = float(numJoints)

        # Tip point dictionary towards
        tipPoint = {'+': [(width / 2.0 * -1), (width / 2.0)],
                    '-': [(width / 2.0), (width / 2.0 * -1)]}
        ...
        # Set the direction of plane according the parallelFaceAxis parallelFaceAxis object point list
        if parallelFaceAxis in direction.keys():
            mc.setAttr(tmpPlane + direction[parallelFaceAxis][0], direction[parallelFaceAxis][1])
        else:
            mc.error('The string %s in parallelFaceAxis argumment is not found. Fill with x or y or z ' % parallelFaceAxis)
        ...
        mc.setAttr((geoPlaneVolume[0] + direction[parallelFaceAxis][2]), -0.5)
        mc.setAttr((geoPlaneVolumeBsn[0] + direction[parallelFaceAxis][2]), -0.5)

        # Create the controllers
        ctrlUp   = ct.Control(prefix =prefix + 'Up', groups_ctrl=['Zro', 'Offset', 'SDK'], ctrl_size=ctrlSize, shape=ctrlTip)
        ctrlMid  = ct.Control(prefix =prefix + 'Mid', groups_ctrl=['Zro', 'Offset', 'SDK'], ctrl_color='blue', ctrl_size=ctrlSize * 1.3, shape=ctrlMid)
        ctrlDown = ct.Control(prefix =prefix + 'Down', groups_ctrl=['Zro', 'Offset', 'SDK'], ctrl_size=ctrlSize, shape=ctrlBase)

        # Match the orientation parent ribbon controller
        ...
        # Set the position of controller to the end of plane
        self.ctrlUpPos   = mc.setAttr((ctrlUp.parent_control[0] + '.translate'), ctrlPos[parallelFaceAxis][0], ctrlPos[parallelFaceAxis][1], ctrlPos[parallelFaceAxis][2])
        self.ctrlDownPos = mc.setAttr((ctrlDown.parent_control[0] + '.translate'), ctrlPos[parallelFaceAxis][3], ctrlPos[parallelFaceAxis][4], ctrlPos[parallelFaceAxis][5])
        ...
        self.twistDef   = self.nonlinearDeformer(objects=[geoPlaneTwist[0]], defType='twist',
                                                 ...
                                                 highBound=1, rotate=rotDefomer[parallelFaceAxis])
        ...
        # set the position for joints wire curve according on point parallelFaceAxis position
        mc.setAttr((self.jointUp[0] + '.translate'), ctrlPos[parallelFaceAxis][0], ctrlPos[parallelFaceAxis][1], ctrlPos[parallelFaceAxis][2])
        mc.setAttr((self.jointMid[0] + '.translate'),self.pos[0], self.pos[1], self.pos[2])
        mc.setAttr((self.jointDown[0] + '.translate'), ctrlPos[parallelFaceAxis][3], ctrlPos[parallelFaceAxis][4], ctrlPos[parallelFaceAxis][5])

        # set the position for joints orientation ribbon
        mc.setAttr((self.jointOrientUp[0] + '.translate'), ctrlPos[parallelFaceAxis][0], ctrlPos[parallelFaceAxis][1], ctrlPos[parallelFaceAxis][2])
        mc.setAttr((self.jointOrientMid[0] + '.translate'),self.pos[0], self.pos[1], self.pos[2])
        mc.setAttr((self.jointOrientDown[0] + '.translate'), ctrlPos[parallelFaceAxis][3], ctrlPos[parallelFaceAxis][4], ctrlPos[parallelFaceAxis][5])
        ...
        cr.direction_pivot(self.jointUpOrientParent[0], aim_axis=aimAxis, up_axis=upAxis)
        cr.direction_pivot(self.jointMidOrientParent[0], aim_axis=aimAxis, up_axis=upAxis)
        cr.direction_pivot(self.jointDownOrientParent[0], aim_axis=aimAxis, up_axis=upAxis)

        # Freezing the joints before skin to the plane
        mc.makeIdentity(self.jointOrientUp[0], self.jointOrientMid[0], self.jointOrientDown[0],
                        self.jointUp[0], self.jointMid[0], self.jointDown[0], apply=True, r=True, n=1)
        ...
        # Freezing the rotation of geoplane and geo volume blendshape
        ...
        mc.connectAttr((ctrlMid.control + '.volumePosition'), (self.squashDef[1] + volInputMdn[parallelFaceAxis][0]))
        ...
        for folS, folV in zip (follicleS['follicle'], follicleV['follicle']):
            ...
            mc.connectAttr((ctrlMid.control + '.volumeMultiplier'), volInputMdn[parallelFaceAxis][1])
            ...
            mc.connectAttr((multMpd + volInputMdn[parallelFaceAxis][2]), (sumPma + '.input1D[0]'))
