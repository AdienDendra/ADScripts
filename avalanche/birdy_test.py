...
def get_value_present():
    ...
    for vtx in present.getShape().vtx:
        ...
        # print start_vector
        move_x = start_vector[0] * pm.dt.sqrt(
            1.0 - (start_vector[1] * start_vector[1] / 2.0) - (start_vector[2] * start_vector[2] / 2.0) +
            (start_vector[1] * start_vector[1] * start_vector[2] * start_vector[2] / 3.0))

        move_y = start_vector[1] * pm.dt.sqrt(
            1.0 - (start_vector[2] * start_vector[2] / 2.0) - (start_vector[0] * start_vector[0] / 2.0) +
            (start_vector[2] * start_vector[2] * start_vector[0] * start_vector[0] / 3.0))

        move_z = start_vector[2] * pm.dt.sqrt(
            1.0 - (start_vector[0] * start_vector[0] / 2.0) - (start_vector[1] * start_vector[1] / 2.0) +
        ...
        return {'move_x':move_x,
                'move_y': move_y,
                'move_z': move_z,
                'start_vector_ws[0]':start_vector_ws[0],
                'start_vector_ws[1]' : start_vector_ws[1],
                'start_vector_ws[2]':start_vector_ws[2],
                'subtract_vec[0]': subtract_vec[0],
                'subtract_vec[1]': subtract_vec[1],
                'subtract_vec[2]': subtract_vec[2]
                }
...
def fix_present():
    ...
    for vtx in present.getShape().vtx:
        ...
        move_x = (start_vector[0] * pm.dt.sqrt(1.0 - (start_vector[1] * start_vector[1] / 2.0) -
                                               (start_vector[2] * start_vector[2] / 2.0) + (start_vector[1] * start_vector[1] * start_vector[2] * start_vector[2] / 3.0)))+ start_vector_ws[0]

        move_y = (start_vector[1] * pm.dt.sqrt(1.0 - (start_vector[2] * start_vector[2] / 2.0) -
                                               (start_vector[0] * start_vector[0] / 2.0) + (start_vector[2] * start_vector[2] * start_vector[0] * start_vector[0] / 3.0)))+ start_vector_ws[1]

        move_z = (start_vector[2] * pm.dt.sqrt(1.0 - (start_vector[0] * start_vector[0] / 2.0) -
                                               (start_vector[1] * start_vector[1] / 2.0) + (start_vector[0] * start_vector[0] * start_vector[1] * start_vector[1] / 3.0)))+ start_vector_ws[2]
        # print (move_x, move_y, move_z)
        new_vector = pm.dt.Vector(move_x, move_y, move_z)
        new_vector = (start_vector_ws - start_vector)
        # new_vector_ws = pm.dt.Vector(position_ws_new_vector, position_ws_new_vector, position_ws_new_vector)

        # subtract_vec = start_vector_ws + new_vector

        # print subtract_vec

        move_x = (new_vector[0] * pm.dt.sqrt(1.0 - (new_vector[1] * new_vector[1] / 2.0) - (new_vector[2] * new_vector[2] / 2.0) +
                                      (new_vector[1] * new_vector[1] * new_vector[2] * new_vector[2] / 3.0)))

        move_y = (new_vector[1] * pm.dt.sqrt(
            1.0 - (new_vector[2] * new_vector[2] / 2.0) - (new_vector[0] * new_vector[0] / 2.0) +
            (new_vector[2] * new_vector[2] * new_vector[0] * new_vector[0] / 3.0)))

        move_z = (new_vector[2] * pm.dt.sqrt(
            1.0 - (new_vector[0] * new_vector[0] / 2.0) - (new_vector[1] * new_vector[1] / 2.0) +
            (new_vector[0] * new_vector[0] * new_vector[1] * new_vector[1] / 3.0)))
